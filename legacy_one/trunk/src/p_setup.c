...
//-----------------------------------------------------------------------------
//
// $Id: p_setup.c 1776 2026-02-07 13:53:48Z wesleyjohnson $
//
...
// [WDJ] num_sectors limited to 0xFFFE, or else reject calc will overflow.
...
void P_LoadVertexes (lumpnum_t lumpnum)
{
    ...
    num_vertexes = W_LumpLength(lumpnum) / sizeof(mapvertex_t);

    // Allocate zone memory for buffer.
    vertexes = Z_Malloc (num_vertexes*sizeof(vertex_t), PU_LEVEL, NULL);
    ...
    uint32_t i;
    for (i=0 ; i<num_vertexes ; i++, li++, ml++)
    {
        // signed
...
void P_LoadSegs ( lumpnum_t lumpnum )
{
    ...
    num_segs = W_LumpLength(lumpnum) / sizeof(mapseg_t);
    segs = Z_Malloc (num_segs*sizeof(seg_t), PU_LEVEL, NULL);
    memset (segs, 0, num_segs*sizeof(seg_t));
...
//        if (rendermode != render_soft)
        ...
        if( linedef > num_lines ) {
            I_SoftError( "P_LoadSegs, linedef #%i, > num_lines %i\n", linedef, num_lines );
            linedef = 0; // default
        }
        ldef = &lines[linedef];
        li->linedef = ldef;
...
void P_LoadSubsectors( lumpnum_t lumpnum )
{
    ...
    num_subsectors = W_LumpLength(lumpnum) / sizeof(mapsubsector_t);
    ...
    ss = subsectors;
    uint32_t i;
    for (i=0 ; i<num_subsectors ; i++, ss++, ms++)
    {
        ss->num_lines = (uint16_t)( LE_SWAP16(ms->num_segs) );
...
unsigned int            num_levelflats;  // actual in use
...
uint16_t P_AddLevelFlat( const char* flatname )
{
    ...
    if( levelflats )
    {
        ...
        lfp = & levelflats[0];
        for (lf_index=0; lf_index<num_levelflats; lf_index++)
        {
    ...
    if (devparm)
        GenPrintf(EMSG_dev, "flat %#03d: %s\n", num_levelflats, name8.s);

    if (num_levelflats>=levelflat_max)
    {
        ...
        memset( &new_levelflats[num_levelflats], 0,
                (levelflat_max - num_levelflats)*sizeof(levelflat_t) );

        if( levelflats )
        {
            // Copy existing levelflats to new memory.
            memcpy (new_levelflats, levelflats, num_levelflats*sizeof(levelflat_t));
            Z_Free ( levelflats );
        }
        levelflats = new_levelflats;
    }

    if( num_levelflats > 0xFFF7 )
        I_Error("number of flats exceeds id size\n" );

    lf_index = num_levelflats++;
...
void P_LoadSectors( lumpnum_t lumpnum )
{
    ...
    num_sectors = W_LumpLength(lumpnum) / sizeof(mapsector_t);
    sectors = Z_Malloc (num_sectors*sizeof(sector_t), PU_LEVEL, NULL);
    memset (sectors, 0, num_sectors*sizeof(sector_t));
    data = W_CacheLumpNum( lumpnum, PU_STATIC );  // mapsector lump temp
    // [WDJ] Fix endian as transfer from temp to internal.

    if( !data || (num_sectors < 1))
    {
...
void P_LoadNodes ( lumpnum_t lump )
{
    ...
    num_nodes = W_LumpLength (lump) / sizeof(mapnode_t);
    nodes = Z_Malloc (num_nodes*sizeof(node_t), PU_LEVEL, NULL);
    data = W_CacheLumpNum (lump,PU_STATIC);  // mapnode_t array temp
    // [WDJ] Fix endian as transfer from temp to internal.

    // No nodes and one subsector is a trivial but legal map.
    if( (!data || (num_nodes < 1)) && (num_subsectors > 1))
    {
    ...
    uint32_t ni;
    for (ni=0 ; ni<num_nodes ; ni++, no++, mn++)
    {
...
#  ifdef DEEPSEA_EXTENDED_SIGNED_NODES
                if( (uint32_t)chld >= num_subsectors )
#  else
                if( chld >= num_subsectors )
#  endif
                {
                ...
                chld |= NF_SUBSECTOR;
#else
                if( (chld & ~MAP_NF_SUBSECTOR) >= num_subsectors )
                {
                    GenPrintf( EMSG_warn, "Load Node %u, side %i has bad child sector %i\n", ni, j, chld );
                    chld = 0 | NF_SUBSECTOR;
                }
...
void P_LoadThings (int lump)
{
  ...
  typedef struct
  {
    ...
    num_mapthings     = W_LumpLength(lump) / sizeof(doom_mapthing_t);
    mapthings        = Z_Malloc(num_mapthings * sizeof(mapthing_t), PU_LEVEL, NULL);

    if( !data || (num_mapthings < 1))
    {
    ...
    uint16_t i;
    for (i=0 ; i<num_mapthings ; i++, mt++, dmt++)
    {
...
void P_LoadLineDefs (int lump)
{
    ...
    num_lines = W_LumpLength (lump) / sizeof(maplinedef_t);
    lines = Z_Malloc (num_lines*sizeof(line_t), PU_LEVEL, NULL);
    memset (lines, 0, num_lines*sizeof(line_t));
    ...
    for (i=0 ; i<num_lines ; i++, mld++, ld++)
    {
        ...
        else if ( ld->sidenum[0] >= num_sides )
        {
...
//              ld->sidenum[1] = 0;  // arbitrary valid sidedef
        ...
        else if ( ld->sidenum[1] >= num_sides )
        {
...
void P_LoadSideDefs (int lump)
{
    ...
    num_sides = W_LumpLength (lump) / sizeof(mapsidedef_t);
    uint32_t sizesides = num_sides*sizeof(side_t);
    ...
    sd = sides;
    uint32_t si;
    for (si=0 ; si<num_sides ; si++, msd++, sd++)
    {
...
{
  num_sides = W_LumpLength(lump) / sizeof(mapsidedef_t);
  sides = Z_Malloc(num_sides*sizeof(side_t), PU_LEVEL, NULL);
  memset(sides, 0, num_sides*sizeof(side_t));
}
...
void P_LoadSideDefs2(int lump)
{
  ...
  uint32_t sdnum;
  for (sdnum=0; sdnum<num_sides; sdnum++)
  {
...
void P_GroupLines (void)
{
    ...
    ss = subsectors;
    uint32_t si;
    for (si=0 ; si<num_subsectors ; si++, ss++)
    {
        // [WDJ] Detct buggy wad
        if( ss->firstline > num_segs )
        {
            I_Error( "P_GroupLines: subsector firstline %i > num_segs %u\n",
                         ss->firstline, num_segs );
        }
    ...
    total = 0;
    uint32_t k;
    for (k=0 ; k<num_lines ; k++, li++) // for each line
    {
    ...
    uint32_t s;
    for (s=0 ; s<num_sectors ; s++, sector++)
    {
        // for each sector
        M_ClearBox (bbox);
...
boolean  P_CheckHeaderLumps( lumpnum_t lump )
{
  ...
  if( filen > num_wadfiles )  goto fail;
  
  int numlump = wadfiles[filen]->numlumps;
...
int  P_CheckLumpName( lumpnum_t lump, byte ml )
{
  ...
  if( filen > num_wadfiles )
      goto fail;

  lumpn = LUMPNUM(lump);
  if( lumpn > wadfiles[filen]->num_lumps )
      goto fail;
...
//    R_Flush_Texture_Cache();
    ...
    // [WDJ] Limits num_sectors to 0xFFFE, or else reject calc will overflow.
...
void  P_process_wadfile( int wadfilenum, /*OUT*/ level_id_t * firstmap_out )
{
    ...
    replace_cnt = 0;
    for (i=0; i<wadfile->num_lumps; i++,lumpinfo++)
    {
        name = lumpinfo->name;
    ...
    //
    R_AddSpriteDefs (sprnames, num_wadfiles-1);
