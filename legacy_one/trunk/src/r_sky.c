...
//-----------------------------------------------------------------------------
//
// $Id: r_sky.c 1756 2025-11-20 11:41:32Z wesleyjohnson $
//
...
// sky mapping
//
uint16_t  sky_texture = 0;

// sky drawing
uint16_t  sky_flatnum;   // to detect sectors with sky
byte *  sky_pict = NULL; // ZMalloc, PU_STATIC
  // TM_picture format, columofs, column image
fixed_t sky_texturemid;
fixed_t sky_scale;
...
byte * R_Generate_Sky( uint16_t texnum )
{
...
void R_Setup_SkyDraw (void)
{
    ...
    sky_texturemid = FIXINT(100);   // Boom
...
//   sky  dc_iscale = FixedDiv (FRACUNIT, pspriteyscale);
    ...
    sky_yh_max_oc = ( (INT_TO_FIXED(sky_height-1)) - sky_texturemid ) / sky_scale;  // frac = sky_height-1
}
