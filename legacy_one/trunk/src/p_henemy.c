...
//-----------------------------------------------------------------------------
//
// $Id: p_henemy.c 1766 2025-11-21 17:46:54Z wesleyjohnson $
//
...
#define MONS_LOOK_RANGE FIXINT(20*64)
...
boolean PH_LookForMonsters(mobj_t * actor)
{
    ...
    {
        if(think->function != TFI_MobjThinker)
        { // Not a mobj thinker
            continue;
        }
        mo = (mobj_t *)think;
        ...
        if(P_AproxDistance_mobj( actor, mo ) > MONS_LOOK_RANGE)
        { // Out of range
            continue;
        }
        ...
        SET_TARGET_REF( actor->target, mo );
        return(true);
    }
    return(false);
}
...
void A_ImpMsAttack(mobj_t * actor)
{
    ...
    actor->momx = FixedMul(FIXINT(12), finecosine[angf]);
    actor->momy = FixedMul(FIXINT(12), finesine[angf]);
...
void A_Feathers(mobj_t * actor)
{
    ...
    {
        // create feathers
        mo = P_SpawnMobj(actor->x, actor->y, actor->z + FIXINT(20), MT_FEATHER);
        SET_TARGET_REF( mo->target, actor );
        mo->momx = PP_SignedRandom(ph_feathers)<<8;
        mo->momy = PP_SignedRandom(ph_feathers)<<8;
        mo->momz = FIXINT(1) + (PP_Random(ph_feathers)<<9);
...
void A_MummySoul(mobj_t* mummy)
{
    ...
    mo = P_SpawnMobj(mummy->x, mummy->y, mummy->z + FIXINT(10), MT_MUMMYSOUL);
    mo->momz = FIXINT(1);
}
...
void A_SorcererRise(mobj_t * actor)
{
    ...
    SET_TARGET_REF( mo->target, actor->target );
}
...
void P_DSparilTeleport(mobj_t * actor)
{
    ...
    } while( P_AproxDistance(actor->x - x, actor->y - y) < FIXINT(128) );
    prev_x = actor->x;
    prev_y = actor->y;
    prev_z = actor->z;
...
void A_BlueSpark(mobj_t * actor)
{
    ...
    for(i = 0; i < 2; i++)
    {
        ...
        mo->momz = FIXINT(1) + (PP_Random(ph_sorsparkmom)<<8);
    }
}
...
void A_MinotaurAtk1(mobj_t * actor)
{
    ...
    if(P_CheckMeleeRange(actor))
    {
        ...
        { // Squish the player
            player->deltaviewheight = FIXINT(-16);
        }
    }
}
...
#define MNTR_CHARGE_SPEED   FIXINT(13)
...
void A_MinotaurDecide(mobj_t * actor)
{
    ...
    dist = P_AproxDistance_mobj( actor, target );
...
void A_HHeadAttack(mobj_t * actor)
{
    ...
    dist = P_AproxDistance_mobj( actor, target ) > FIXINT(8 * 64);
    ...
    { // Fire column
        ...
        if(baseFire != NULL)
        {
            ...
            for(i = 0; i < 5; i++)
            {
                ...
                SET_TARGET_REF( fire->target, baseFire->target );
...
void A_HeadIceImpact(mobj_t* ice)
{
    ...
    for(i = 0; i < 8; i++)
    {
        ...
        shard->angle = angle;
#ifdef MONSTER_VARY
        shard->momx = FixedMul(shard->speed, cosine_ANG(angle));
        shard->momy = FixedMul(shard->speed, sine_ANG(angle));
#else
        shard->momx = FixedMul(shard->info->speed, cosine_ANG(angle));
        shard->momy = FixedMul(shard->info->speed, sine_ANG(angle));
#endif
        shard->momz = FIXFL(-0.6);
...
void P_DropItem(mobj_t* source, mobjtype_t type, int special, int chance)
{
    ...
    mo->momz = FIXINT(5) + (PP_Random(ph_dropmom)<<10);
    mo->flags |= MF_DROPPED;
    mo->health = special;
}
...
void A_PodPain(mobj_t * actor)
{
    ...
    for(i = 0; i < count; i++)
    {
        goo = P_SpawnMobj(actor->x, actor->y, actor->z + FIXINT(48),
                          MT_PODGOO );
        goo->target = actor;
...
void A_MakePod(mobj_t * actor)
{
    ...
    P_ThrustMobj(mo, PP_Random(ph_podthrust)<<24, FIXFL(4.5));
...
void A_SpawnTeleGlitter(mobj_t * actor)
{
    ...
    mo = P_SpawnMobj(actor->x + INT_TO_FIXED((r&31)-16),
                     actor->y + INT_TO_FIXED((s&31)-16),
...
void A_InitKeyGizmo(mobj_t * gizmo)
{
    ...
    mo = P_SpawnMobj(gizmo->x, gizmo->y, gizmo->z + FIXINT(60), MT_KEYGIZMOFLOAT);
...
void A_VolcanoBlast(mobj_t * volcano)
{
    ...
    for(i = 0; i < count; i++)
    {
        blast = P_SpawnMobj(volcano->x, volcano->y, volcano->z + FIXINT(44),
                            MT_VOLCANOBLAST ); // MT_VOLCANOBLAST
        blast->target = volcano;
        angle = PP_Random(ph_volcanobl)<<24;
        blast->angle = angle;
        blast->momx = FixedMul(FIXINT(1), cosine_ANG(angle));
        blast->momy = FixedMul(FIXINT(1), sine_ANG(angle));
        blast->momz = FIXFL(2.5) + (PP_Random(ph_volcanobl)<<10);
...
void A_VolcBallImpact(mobj_t * ball)
{
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        angle = i*ANG90;
        tiny->angle = angle;
        tiny->momx = FixedMul( FIXFL(0.7), cosine_ANG(angle));
        tiny->momy = FixedMul( FIXFL(0.7), sine_ANG(angle));
        tiny->momz = FIXINT(1) + (PP_Random(ph_volcanohit)<<9);
...
void A_SkullPop(mobj_t * actor)
{
    ...
    actor->momz = FIXINT(2) + (PP_Random(ph_bloodyskullmom)<<6);
    actor->health = health;
#else
    // Spawn bloody skull as separate object
    // Allows player to see bloody skull.
    mobj_t * mo;
    mo = P_SpawnMobj(actor->x, actor->y, actor->z + FIXINT(48), MT_BLOODYSKULL);
    //mo->target = actor;
    mo->momx = PP_SignedRandom(ph_bloodyskullmom)<<9;
    mo->momy = PP_SignedRandom(ph_bloodyskullmom)<<9;
    mo->momz = FIXINT(2) + (PP_Random(ph_bloodyskullmom)<<6);
    mo->angle = actor->angle;
