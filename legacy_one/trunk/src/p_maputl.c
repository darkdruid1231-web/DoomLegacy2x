...
//-----------------------------------------------------------------------------
//
// $Id: p_maputl.c 1761 2025-11-20 11:48:04Z wesleyjohnson $
//
...
{
    fixed_t dx = abs(mo1->x - mo2->x);
    fixed_t dy = abs(mo1->y - mo2->y);
    if (dx < dy)
        return dx+dy-(dx>>1);
    return dx+dy-(dy>>1);
}
...
int  P_PointOnLineSide ( fixed_t x, fixed_t y, const line_t* line )
{
    ...
    // Shift is to prevent overflow, scaled result.
                         ...
                         traverser_t   trav)
{
    ...
    for (count = 0 ; count < 64 ; count++)
    {
        ...
        if ( FIXED_TO_INT(yintercept) == mapy)
        {
            yintercept += ystep;
            mapx += mapxstep;
        }
        else if ( FIXED_TO_INT(xintercept) == mapx)
        {
            xintercept += xstep;
            mapy += mapystep;
        }
