...
LArchive &LArchive::operator<<(char *&s)
{
    ...
    {
        s = Z_Strdup(reinterpret_cast<char *>(m_pos),
                     PU_STATIC,
                     NULL); // there better be a terminating '\0' !
        m_pos += strlen(s) + 1;
    }
...
LArchive &LArchive::operator<<(Material *&m)
{
    ...
    else
    {
        ...
        m = materials.Get(reinterpret_cast<char *>(m_pos),
                          TEX_wall); // there better be a terminating '\0' !
