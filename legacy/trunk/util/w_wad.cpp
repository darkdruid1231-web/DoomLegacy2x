...
// Not like libc access(). If file exists, returns the path+filename where it was found, otherwise
// NULL.
...
int FileCache::AddFile(const char *fname, bool silent)
{
    ...
    {
        CONS_Printf(
            " FileCache::AddFile: Can't access file %s (path %s)\n", fname, datapath.c_str());
        return -1;
    }
...
int FileCache::FindNumForNameFile(const char *name, unsigned filenum, int startlump)
{
    ...
    if (res != -1)
        return ((filenum << 16) + res);

    // not found.
    return -1;
}

// Return the first lump in a file whose name starts with the given four bytes, also returns the
// full name of the lump.
