...
flag_mnemonic_t BEX_FlagMnemonics[] = {
    ...
    {"NOTARGET",
     MF2_NOTARGET,
     2}, // Will not be targeted by other monsters of same team (like Arch-Vile)
...
string_mnemonic_t BEX_StringMnemonics[] = {
    BEX_STR(GOTARMOR) BEX_STR(GOTMEGA) BEX_STR(GOTHTHBONUS) BEX_STR(GOTARMBONUS) BEX_STR(GOTSTIM)
        BEX_STR(GOTMEDINEED) BEX_STR(GOTMEDIKIT) BEX_STR(GOTSUPER) BEX_STR(GOTINVUL)
            BEX_STR(GOTBERSERK) BEX_STR(GOTINVIS) BEX_STR(GOTSUIT) BEX_STR(GOTMAP) BEX_STR(GOTVISOR)
                BEX_STR(GOTMSPHERE) BEX_STR(GOTCLIP) BEX_STR(GOTCLIPBOX) BEX_STR(GOTROCKET)
                    BEX_STR(GOTROCKBOX) BEX_STR(GOTCELL) BEX_STR(GOTCELLBOX) BEX_STR(GOTSHELLS)
                        BEX_STR(GOTSHELLBOX) BEX_STR(GOTBACKPACK) BEX_STR(GOTBFG9000)
                            BEX_STR(GOTCHAINGUN) BEX_STR(GOTCHAINSAW) BEX_STR(GOTLAUNCHER)
                                BEX_STR(GOTPLASMA) BEX_STR(GOTSHOTGUN) BEX_STR(GOTSHOTGUN2)

                                    BEX_STR(PD_BLUEO) BEX_STR(PD_YELLOWO) BEX_STR(PD_REDO) BEX_STR(
                                        PD_BLUEK) BEX_STR(PD_YELLOWK) BEX_STR(PD_REDK)

                                        BEX_STR(CC_ZOMBIE) BEX_STR(CC_SHOTGUN) BEX_STR(CC_HEAVY)
                                            BEX_STR(CC_IMP) BEX_STR(CC_DEMON) BEX_STR(CC_LOST)
                                                BEX_STR(CC_CACO) BEX_STR(CC_HELL) BEX_STR(CC_BARON)
                                                    BEX_STR(CC_ARACH) BEX_STR(CC_PAIN)
                                                        BEX_STR(CC_REVEN) BEX_STR(CC_MANCU)
                                                            BEX_STR(CC_ARCH) BEX_STR(CC_SPIDER)
                                                                BEX_STR(CC_CYBER) BEX_STR(CC_HERO)

    // Boom messages.
    BEX_STR(PD_BLUEC) BEX_STR(PD_YELLOWC) BEX_STR(PD_REDC) BEX_STR(PD_BLUES) BEX_STR(PD_YELLOWS)
        BEX_STR(PD_REDS) BEX_STR(PD_ANY) BEX_STR(PD_ALL3) BEX_STR(PD_ALL6)
