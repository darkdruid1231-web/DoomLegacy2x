...
class InMemorySerializer : public ISerializer
{
  public:
    ...
    explicit InMemorySerializer(size_t reserve) : buffer(), bitPos(0), writing_(true)
    {
        buffer.reserve(reserve);
    }
    /// Explicit direction constructor.
    static InMemorySerializer forReading()
    {
        InMemorySerializer s;
        s.writing_ = false;
        return s;
    }
    ...
    void write(uint32_t val, size_t bits)
    {
        write(&val, bits);
    }
