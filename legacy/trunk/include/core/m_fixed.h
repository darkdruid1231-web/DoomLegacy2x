...
class fixed_t
{
  ...
  public:
    ...
    inline fixed_t(double f)
    {
        val = value_t(f * double(UNIT));
    }

    // copy constructor (has to be light!)
    inline fixed_t(const fixed_t &f)
    {
        val = f.val;
    }
    ...
    template <typename U> inline fixed_t &operator*=(const U &a)
    {
        val = value_t(val * a);
        return *this;
    };
    ...
    /// Allow a non-fixed_t object on the left for efficiency (specialized fixed_t-fixed_t version
    /// follows!)
    // template<typename U>
    ...
    /// Division template.
    /// Allow a non-fixed_t object on the right for efficiency (specialized fixed_t-fixed_t version
    /// follows!)
    template <typename U> inline friend fixed_t operator/(const fixed_t &a, const U &b)
    {
        fixed_t res;
    ...
    inline friend bool operator==(const fixed_t &a, const fixed_t &b)
    {
        return a.val == b.val;
    }
    inline friend bool operator!=(const fixed_t &a, const fixed_t &b)
    ...
    /// basic functions
    inline friend fixed_t abs(const fixed_t &a)
    {
        fixed_t res;
        res.val = abs(a.val);
        return res;
    }
    inline friend fixed_t sqrt(const fixed_t &a)
    {
        fixed_t res;
        res.val = int(sqrtf(a.val) * 256.0);
        return res;
    }
    ...
    {
        return float(val) / float(UNIT);
    }

    // NOTE: the rounding funcs assume that right-shifting negative values continues the sign, ie.
    // (-1 >> n) == -1. In other words we assume a two's complement representation for negative
    // numbers...
    /// round towards zero
    inline value_t trunc() const
    {
        return (val >= 0) ? val >> FBITS : ((val - 1) >> FBITS) + 1; /* return val >> FBITS; */
    }
    /// round towards -infinity
    inline value_t floor() const
    {
        return val >> FBITS; /* return (val >= 0) ? val >> FBITS : ((val+1) >> FBITS)-1; */
    }
    /// round towards +infinity
    inline value_t ceil() const
    {
        return ((val - 1) >> FBITS) +
               1; /* return (val > 0) ? ((val-1) >> FBITS)+1 : val >> FBITS; */
    }

    /// returns the fractional part of _nonnegative_ number
    inline fixed_t frac() const
    {
        fixed_t res;
        res.val = val & FMASK;
        return res;
    ...
    inline large_t value() const
    {
        return val;
    }
    inline fixed_t &setvalue(value_t v)
    {
        val = v;
        return *this;
    }
...
template <> inline fixed_t &fixed_t::operator*=(const fixed_t &a)
{
