...
//-----------------------------------------------------------------------------
//
// $Id: g_map.h 526 2008-01-22 23:28:24Z smite-meister $
//
...
#define MAXRADIUS 32
    ...
    /// Without the "Reject special effects" hacks in some PWADs, this could be used as a PVS lookup
    /// as well.
    //@{
    byte *rejectmatrix;
    //@}
...
#define ACS_MAP_VARS 32
    ...
    class acs_t *
    ACS_StartScript(unsigned number, byte *args, Actor *activator, line_t *line, int side);
    ...
    Thinker
        thinkercap; ///< Linked list of Thinkers in the map. The head and tail of the thinker list.
    ...
    multimap<int, mapthing_t *> playerstarts; ///< Mapping from player number to playerstart.
                                              ///< args[0] holds the entrypoint number.
  ...
  public:
    ...
    PlayerInfo *
    FindPlayer(int number); // returns player 'number' if he is in the map, otherwise NULL
    int RespawnPlayers();
    int HandlePlayers();
    ...
    inline DActor *SpawnDActor(const vec_t<fixed_t> &r, mobjtype_t t)
    {
        return SpawnDActor(r.x, r.y, r.z, t);
    }
    inline DActor *SpawnDActor(const vec_t<fixed_t> &r, const ActorInfo *ai)
    {
        return SpawnDActor(r.x, r.y, r.z, ai);
    }
    void SpawnPlayer(PlayerInfo *pi, mapthing_t *mthing);
    DActor *SpawnSplash(const vec_t<fixed_t> &pos,
                        fixed_t z,
                        int sound,
                        mobjtype_t base,
                        mobjtype_t chunk = MT_NONE,
                        bool randtics = true);
    ...
    int EV_SpawnLight(
        unsigned tag, int type, short maxl, short minl = 0, short maxt = 0, short mint = 0);
    ...
    int
    EV_DoCeiling(unsigned tag, line_t *line, int type, fixed_t speed, int crush, fixed_t height);
    int EV_DoCrusher(
        unsigned tag, int type, fixed_t upspeed, fixed_t downspeed, int crush, fixed_t height);
    ...
    int EV_BuildHexenStairs(unsigned tag,
                            int type,
                            fixed_t speed,
                            fixed_t stepdelta,
                            int resetdelay,
    ...
    int EV_DoElevator(unsigned tag,
                      int type,
                      fixed_t speed,
                      fixed_t height_f,
                      fixed_t height_c = 0,
    ...
    bool EV_ActivatePolyobj(
        unsigned id, int type, float speed, angle_t angle, float dist, int delay, bool override);

    // in p_things.cpp
    bool EV_ThingProjectile(
        int tid, const ActorInfo *ai, angle_t angle, float hspeed, float vspeed, bool gravity);
  ...
  public:
    ...
    class Iterate_TID
    {
      ...
      public:
        ...
        inline Actor *Next()
        {
            return (low == high) ? NULL : (low++)->second;
        }
    };
};
