...
// The normal way to invoke a thinker descendant is to call its "function" (acp1) with one
// parameter, a pointer to the class itself. This is just like a C++ method call passing the
// implicit 'this' pointer to a method.

// mobj_t : one function pointer in thinker_t (always P_MobjThinker), another available through
// state,
//     called _by_ P_MobjThinker.
// What makes mobjs different from one another is their initial attributes and their state cycle
// ("AI"). in short, in the case of player mobjs, mobj_t.state is used to implement weapon state
// control and simple sequences where no weapon is required (like dying). Otherwise it is used to
// implement the mobj AI.
...
class TypeInfo
{
  private:
    ...
    static map<unsigned, TypeInfo *> &
    id_map(); ///< mapping from class ID numbers to TypeInfo instances
...
#define IMPLEMENT_CLASS(c, par) TypeInfo c::_type(#c, c::Create, &par::_type);
...
class Thinker
{
  ...
  public:
    ...
    inline Thinker *Next() const
    {
        return next;
    };
    inline Thinker *Prev() const
    {
        return prev;
    };
