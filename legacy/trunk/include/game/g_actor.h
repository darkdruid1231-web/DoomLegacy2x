...
enum mobjflag_t
{
    ...
    MF_NOSCORCH = MF_NOBLOOD, ///< TEST flag double use for MF_MISSILEs, which cannot be hit (and
                              ///< thus cannot bleed)
                              // spawning
    ...
    MF_NORESPAWN =
        0x00100000, ///< Will not respawn after being picked up. Pretty similar to MF_DROPPED?
                    // classification
    MF_COUNTKILL = 0x00200000, ///< On kill, count this enemy object towards intermission kill
                               ///< total. Happy gathering.
    MF_COUNTITEM =
        0x00400000, ///< On picking up, count this item object towards intermission item total.
    ...
    MF_MISSILE = 0x02000000, ///< Player missiles as well as fireballs. Don't hit same species,
                             ///< explode on block.
    MF_CORPSE = 0x04000000,  ///< Dead. Acts like a corpse, falls down stairs etc.
    MF_MONSTER = 0x08000000, ///< Monster. Possible threat, can activate some SPAC_CROSS and
                             ///< SPAC_USE lines etc.
    MF_PLAYER = 0x10000000,  ///< Treated as a player. Possible threat, can activate SPAC_CROSS and
                             ///< SPAC_USE lines.
    MF_VALIDTARGET =
...
enum mobjflag2_t
{
    ...
    MF2_NOTARGET =
        0x0800, ///< Will not be targeted by other monsters of the same team (like Arch-Vile)
    ...
    MF2_BOSS =
        0x200000, ///< Is a major boss, not as easy to kill (odd collection of immunities)
                  // dragon, sorcboss,korax, spidermm, cyborg, both dsparils, minotaur
                  //  full vol seesound and activesound, cannot be blasted, some resistance to
                  //  wraithverge, not affected by others blasted to them, some res to
                  //  magelightning, some res to bloodscourge, doom and heretic bosses take no
                  //  radiusdamage, no morph or teleportother heretic and hexen bosses do not
                  //  aggravate teammates ever hexen class bosses: some res to bloodsc., no morphing
    ...
    // rendering
    MF2_FOOTCLIP =
        0x01000000, ///< Feet should be be clipped by floorclip units when standing on liquid floor.
...
class Actor : public Thinker, public NetObject
{
  ...
  public:
    ...
    /// quick z positions
    inline fixed_t Top() const
    {
        return pos.z + height;
    }
    inline fixed_t Center() const
    {
        return pos.z + (height >> 1);
    }
    inline fixed_t Feet() const
    {
        return pos.z;
    }
    ...
    int reactiontime; ///< Time (in tics) before the thing can attack or move again. For
                      ///< MF2_FLOATBOB actors this is the bob phase.
  ...
  public:
    ...
    // Actor(const Actor &a); // TODO is the auto-generated copy constructor bad because of
    // NetObject fields?
    virtual ~Actor();
    ...
    inline bool TestLocation()
    {
        return TestLocation(pos);
    }
  ...
  public:
    ...
    void RadiusAttack(
        Actor *culprit, int damage, fixed_t radius = -1, int dtype = dt_normal, bool downer = true);
...
class DActor : public Actor
{
  ...
  public:
    ...
    inline bool SetState(statenum_t snum, bool call = true)
    {
        return SetState(&states[snum], call);
    };
