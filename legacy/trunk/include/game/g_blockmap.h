...
#define MAPBLOCK_END static_cast<Uint16>(0xFFFF) ///< terminator for a blocklist
    ...
    inline int BlockX(fixed_t x) const
    {
        return (x - orgx).floor() >> MAPBLOCKBITS;
    }
    inline int BlockY(fixed_t y) const
    {
        return (y - orgy).floor() >> MAPBLOCKBITS;
    }
  ...
  public:
    class Map *parent_map; ///< TODO unnecessary if we used line_t*'s instead of Uint16 indices in
                           ///< the blocklists...
    ...
    bool
    PathTraverse(const vec_t<fixed_t> &p1, const vec_t<fixed_t> &p2, int flags, traverser_t trav);

    inline fixed_t FracX(fixed_t x) const
    {
        return (x - orgx) % MAPBLOCKUNITS;
    }
    inline fixed_t FracY(fixed_t y) const
    {
        return (y - orgy) % MAPBLOCKUNITS;
    }
};
