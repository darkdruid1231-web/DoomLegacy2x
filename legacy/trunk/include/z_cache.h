...
//-----------------------------------------------------------------------------
//
// $Id: z_cache.h 536 2009-06-29 06:46:13Z smite-meister $
//
  ...
  public:
    ...
    inline void MakeLink()
    {
        usefulness = -usefulness - 1; // HACK negative usefulness marks them as links, -1 denotes
                                      // link with zero usefulness etc.
    }

    /// Mark item used by incrementing refcount. Return true if item is a link, i.e. usefulness is
    /// negated.
    inline bool AddRef()
    {
        refcount++;
...
template <typename T> class cachesource_t : public HashDictionary<T>
{
  ...
  public:
    ...
    {
        int k = 0;
        for (typename parent::dict_iter_t s = parent::dict_map.begin();
             s != parent::dict_map.end();)
        {
