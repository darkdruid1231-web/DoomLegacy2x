...
template <typename T> class HashDictionary
{
  protected:
    ...
    // the bastards changed semantics when they replaced hash_map with unordered_map! we need to
    // have our own hash functor!
  ...
  public:
    ...
    /// Since hash_map is a unique associative container we need this, cannot just Insert new stuff
    /// with same key. \return 1 if an old element was replaced, 0 if not.
    ...
    void Clear()
    {
        ...
        {
            dict_iter_t s = t++; // because... well, incrementing the iterator apparently uses its
                                 // name, which must not yet be deleted.
            delete s->second;
        }
