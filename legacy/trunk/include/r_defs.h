...
//-----------------------------------------------------------------------------
//
// $Id: r_defs.h 518 2007-12-31 02:54:58Z smite-meister $
//
...
struct vertex_t
{
    ...
    inline vertex_t &operator+=(const vertex_t &a)
    {
        x += a.x;
        y += a.y;
        return *this;
...
struct sector_t
{
    ...
    int teamstartsec; ///< if nonzero, spawnpoints in this sector are only for the corresponding
                      ///< team
    ...
    inline bool SkyFloor() const
    {
        return !floorpic;
    }
    inline bool SkyCeiling() const
    {
        return !ceilingpic;
    }
  ...
  public:
    ...
    /// Shrinks and chops up the range in by Z-planes in sector, returns low-to-high sorted list of
    /// ranges.
...
struct divline_t
{
    ...
    /// Returns the fractional length along the divline where the other divline intercepts it, or 0
    /// if parallel.
