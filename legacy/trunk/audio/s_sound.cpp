...
sounditem_t::sounditem_t(const char *n) : cacheitem_t(n)
{
}
  ...
  protected:
    ...
    virtual sounditem_t *Load(const char *p)
    {
        ...
        if (!strncmp(h->riff_magic, "RIFF", 4))
        {
            ...
            if (LONG(h->riff_size) + 8 != size || strncmp(h->fmt.magic, "fmt ", 4) ||
                strncmp(h->data.magic, "data", 4))
                I_Error("WAVE lump '%s' is corrupted!\n", p);

            t->depth = SHORT(h->fmt.bitspersample) / 8;

            if (LONG(h->fmt.size) != 16 || SHORT(h->fmt.audioformat) != 1 // PCM only
