...
HWBsp::HWBsp(int size, int bspnum) : num_planepolys(size)
{
    ...
    // TODO: this is already done somewhere in the software renderer, maybe we should keep that
...
void HWBsp::AddSubsector(unsigned int num, Poly *poly)
{
    if (poly)
    {
        poly = CutOutSubsecPoly(
            &R.segs[R.subsectors[num].first_seg], R.subsectors[num].num_segs, poly);
                      ...
                      Poly **backpoly)  // return the other here
{
    ...
    PolyVertex vs, ve;
    float fracs = 0.0,
          frace = 0.0; // used to tell which poly is on the front side of the bsp partition line
    int ps = -1, pe = -1;
...
Subsector::Subsector(int num, Poly *poly)
{
    ...
    {
        locFloorHeight = frontsector->virtualFloorheight;
        locCeilingHeight = frontsector->virtualCeiling ? frontsector->virtualCeilingheight
    ...
    if (frontsector->ffloors)
    {
        ...
        floorlightlevel =
            *frontsector->lightlist[R_GetPlaneLight(frontsector, locFloorHeight, false)].lightlevel;
        ceilinglightlevel =
            *frontsector->lightlist[R_GetPlaneLight(frontsector, locCeilingHeight, false)]
                 .lightlevel;
    }

    // We have now all the info we need to create the different arrays (colors, texcoords and
    // vertices)

    State *state = new State();
    state->SetColor(
        (rand() % 256) / 255.0f, (rand() % 256) / 255.0f, (rand() % 256) / 255.0f, 1.0f);
    if (poly->numpts > 2)
    {
        // -= FLOOR =-
    ...
    if (line) // FIXME: this probably doesn't work with 3D-floors yet
    {
        while (count--)
        {
            if (line->backsector)
            {
                if ((locFloorHeight < line->backsector->floorheight) &&
                    (locCeilingHeight > line->backsector->floorheight))
                {
                    AddWall(line,
                            prev_line,
                            next_line,
                            locFloorHeight,
                            line->backsector->floorheight); // bottom
                }
                if ((locCeilingHeight > line->backsector->ceilingheight) &&
                    (locFloorHeight < line->backsector->ceilingheight))
                {
                    AddWall(line,
                            prev_line,
                            next_line,
                            line->backsector->ceilingheight,
                            locCeilingHeight); // top
                }
            }
            else
            {
...
void Subsector::AddWall(
    seg_t *line, seg_t *prev_line, seg_t *next_line, fixed_t floor, fixed_t ceiling)
{
    Geometry *geometry = new Geometry();
    State *state = new State();
    state->SetColor(
...
void Subsector::Render()
{
    /*
      sector_t tempsec;
      int floorlightlevel, ceilinglightlevel;
      frontsector = R.R_FakeFlat(sub->sector, &tempsec, &floorlightlevel, &ceilinglightlevel,
      false); if (frontsector->moved)
        {
