...
void Rend::R_RenderMaskedSegRange(drawseg_t *ds, int x1, int x2)
{
    ...
    else if (ldef->special == LINE_LEGACY_EXT &&
             ldef->args[0] == LINE_LEGACY_RENDERER) // HACK fog sheet
    {
    ...
    {
        colfunc_2s =
            R_Render2sidedMultiPatchColumn; // render multipatch with no holes (no post_t info)
    ...
    if (frontsector->numlights)
    {
        ...
        if (dc_numlights >= dc_maxlights)
        {
            ...
            dc_lightlist =
                (r_lightlist_t *)realloc(dc_lightlist, sizeof(r_lightlist_t) * dc_maxlights);
        }
        ...
        for (i = 0; i < dc_numlights; i++)
        {
            ...
            dc_lightlist[i].flags =
                frontsector->lightlist[i].caster ? frontsector->lightlist[i].caster->flags : 0;
        }
    }
    else
    {
...
#warning GetColumn should get a fixed_t param
            ...
            if (dc_numlights)
            {
                ...
                for (i = 0; i < dc_numlights; i++)
                {
                    ...
                    if (dc_lightlist[i].flags & FF_FOG ||
                    ...
                    if (dc_lightlist[i].extra_colormap && !fixedcolormap)
                        dc_lightlist[i].rcolormap =
                            dc_lightlist[i].extra_colormap->colormap + xwalllights[index];
...
void Rend::R_RenderThickSideRange(drawseg_t *ds, int x1, int x2, ffloor_t *ffloor)
{
    ...
    else
    {
        ...
        if ((frontsector->extra_colormap && frontsector->extra_colormap->fog))
            ...
            lightnum =
                (R_FakeFlat(frontsector, &tempsec, &templight, &templight, false)->lightlevel >>
                 LIGHTSEGSHIFT) +
                extralight;
...
#warning GetColumn should get a fixed_t param
            ...
            if (dc_numlights)
            {
                ...
                for (i = 0; i < dc_numlights; i++)
                {
                    ...
                    if (lighteffect)
                    {
                        if (ffloor->flags & FF_FOG || dc_lightlist[i].flags & FF_FOG ||
                            (dc_lightlist[i].extra_colormap && dc_lightlist[i].extra_colormap->fog))
                            lightnum = (ffloor->master->frontsector->lightlevel >> LIGHTSEGSHIFT);
                        else
                            lightnum = (dc_lightlist[i].lightlevel >> LIGHTSEGSHIFT) + extralight;
                    ...
                    else if (dc_lightlist[i].flags & FF_CUTEXTRA && ffloor->flags & FF_EXTRA)
                    {
                        if (dc_lightlist[i].flags & FF_EXTRA)
                        {
                            ...
                            if ((dc_lightlist[i].flags & (FF_TRANSLUCENT | FF_FOG)) ==
...
#undef ADJUST
    ...
    else
    {
        ...
        if (worldhigh < worldtop)
        {
            ...
            if (waterplane && waterz > worldhigh && waterz < worldtop)
                markwater = true;
#endif
        ...
        if (worldlow > worldbottom) // seulement si VISIBLE!!!
        {
            ...
            if (waterplane && waterz < worldlow && waterz > worldbottom)
                markwater = true;
#endif
            ...
            (backsector->ffloors || frontsector->ffloors))
        {
            ...
            lowcut = frontsector->floorheight > backsector->floorheight ? frontsector->floorheight
                                                                        : backsector->floorheight;
            highcut = frontsector->ceilingheight < backsector->ceilingheight
                          ? frontsector->ceilingheight
                          : backsector->ceilingheight;
            ...
            {
                i = 0;
                for (rover = backsector->ffloors; rover != NULL && i < MAXFFLOORS;
                     rover = rover->next)
                {
    ...
    if (frontsector->numlights)
    {
        ...
        if (curline->numlights < dc_numlights)
        {
            ...
            curline->rlights =
                (r_lightlist_t *)Z_Malloc(sizeof(r_lightlist_t) * dc_numlights, PU_LEVEL, 0);
    ...
    if (linedef->splats && cv_splats.value)
    {
        ...
        memcpy(last_ceilingclip + ds_p->x1,
               ceilingclip + ds_p->x1,
               sizeof(short) * (ds_p->x2 - ds_p->x1 + 1));
        memcpy(last_floorclip + ds_p->x1,
               floorclip + ds_p->x1,
               sizeof(short) * (ds_p->x2 - ds_p->x1 + 1));
        R_RenderSegLoop();
        R_DrawWallSplats();
