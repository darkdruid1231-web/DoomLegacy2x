...
void spritepres_t::Project(Actor *p)
{
    ...
    else
    {
        if (flags & TFF_TRANSMASK)
            vis->transmap = transtables[((flags & TFF_TRANSMASK) >> TFF_TRANSSHIFT) - 1];
                 ...
                 (MF_SHADOW |
                  MF_ALTSHADOW)) // TODO altshadow should transpose the translucency table...
    ...
    {
        if (sec->lightlist[i].height >= vis->gzt ||
            !(sec->lightlist[i].caster->flags & FF_CUTSPRITES))
            continue;
...
void R_DrawMaskedColumn(column_t *column)
{
    ...
    for (; column->topdelta != 0xff;)
    {
        ...
        fixed_t bottomscreen =
            sprbotscreen == fixed_t::FMAX ? topscreen + column->length / dc_iscale :
    ...
    if (!(ll->caster->flags & FF_NOSHADE))
    {
        ...
        else
        {
            ...
            */
            ...
            if (fixedcolormap)
                ;
            else if ((thing->pres->flags & (TFF_FULLBRIGHT | TFF_TRANSMASK) ||
...
void Rend::R_DrawPSprite(pspdef_t *psp)
{
    ...
    */
    ...
    if (viewplayer->subsector->sector->numlights)
    {
        ...
        lightnum = (*viewplayer->subsector->sector->lightlist[light].lightlevel >> LIGHTSEGSHIFT) +
                   extralight;
...
void Rend::R_DrawPlayerSprites()
{
    ...
    if (viewplayer->subsector->sector->numlights)
    {
        ...
        lightnum =
            (*viewplayer->subsector->sector->lightlist[0].lightlevel >> LIGHTSEGSHIFT) + extralight;
    }
    else
    ...
    for (vissprite_t *rover = vsprsortedhead.prev; rover != &vsprsortedhead; rover = rover->prev)
    {
        ...
        for (r2 = nodehead.next; r2 != &nodehead; r2 = r2->next)
        {
            ...
            else if (r2->thickseg)
            {
                ...
                scale = r2->thickseg->scale1 > r2->thickseg->scale2 ? r2->thickseg->scale1
                                                                    : r2->thickseg->scale2;
...
void Rend::R_DrawSprite(vissprite_t *spr)
{
    ...
    for (drawseg_t *ds = ds_p; ds-- > drawsegs;)
    {
        ...
        if (scale < spr->yscale ||
            (lowscale < spr->yscale && !divline_t(ds->curline).PointOnSide(spr->px, spr->py)))
        {
            // masked mid texture?
            /*if (ds->maskedtexturecol)
