...
//-----------------------------------------------------------------------------
//
// $Id: png.cpp 536 2009-06-29 06:46:13Z smite-meister $
//
...
PNGTexture::PNGTexture(const char *n, int l) : LumpTexture(n, l, 0, 0)
{
...
bool PNGTexture::ReadData(bool read_image, bool sw_rend)
{
    ...
    // color_type, png_get_rowbytes(png_p, info_p), info_p->channels);
    ...
    else
    {
        ...
        */
        ...
        if (sw_rend)
        {
            ...
            RGB_t *rgb_buf =
                static_cast<RGB_t *>(Z_Malloc(w * h * sizeof(RGB_t), PU_TEXTURE, NULL));
            ...
            // this takes some time, so better do it only once (never flush PNG textures
            // automatically) colormap and transpose to col-major order
            unsigned dest = 0;
            for (i = 0; i < len; i++)
            {
        ...
        else
        {
            ...
            if (!(color_type & PNG_COLOR_MASK_ALPHA))
                png_set_filler(
                    png_p, 0xFF, PNG_FILLER_AFTER); // add opaque alpha channel if none present
...
bool WritePNGScreenshot(FILE *fp, byte *lfb, int width, int height, RGB_t *pal)
{
    ...
    png_struct *png_p =
        png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL, PNG_error, PNG_warning);
    if (!png_p)
        return false;
