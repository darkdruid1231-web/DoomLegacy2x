...
//-----------------------------------------------------------------------------
//
// $Id: r_sprite.cpp 533 2009-06-11 08:31:10Z smite-meister $
//
...
skin_t::skin_t(const char *n, const char *spritename, const char *fprefix) : cacheitem_t(n)
{
...
void R_AddSkins(int wadnum)
{
  ...
  wadnum);
            ...
            if(!found)
          I_Error("R_AddSkins: Unknown keyword '%s' in S_SKIN lump# %d (WAD
...
sprite_t::sprite_t(const char *n) : cacheitem_t(n)
{
    iname = numframes = 0;
    spriteframes = NULL;
}
...
void spritepres_t::SetFrame(const state_t *st)
{
    ...
    lastupdate = -1; // hack, since it will be Updated on this same tic TODO lastupdate should be
                     // set to nowtic
}
