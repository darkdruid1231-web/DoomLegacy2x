...
//-----------------------------------------------------------------------------
//
// $Id: r_draw.cpp 504 2007-12-16 17:58:26Z smite-meister $
//
...
/*!
  ...
  The column drawing routines of the software renderer handle the drawing of wall textures and
  sprites to the framebuffer. They use a number of global variables starting with "dc_" for
  parameter passing.

  The texturemapping for the i:th pixel in the column is given by
  ylookup[dc_yl+i][columnofs[dc_x]] = dc_colormap[dc_source[(dc_texturemid +
  (dc_yl+i-centery)*dc_iscale) % dc_texheight]];

  *R_DrawColumn_8: basic
  *R_DrawFuzzColumn_8: messes with dc_yl, dc_yh, uses dest[fuzzoffset[fuzzpos]] as source, maps it
  with lighttable 6 *R_DrawTranslucentColumn_8: adds a dc_transmap[source][dest] mapping before the
  final dc_colormap R_DrawShadeColumn_8: chooses lightlevel colormap depending on source pixel,
  applies it on dest. What if source pixel >= 34? R_DrawTranslatedColumn_8: adds a dc_translation
  colormap before the final dc_colormap *R_DrawFogColumn_8: applies dc_colormap to dest, no source
...
/*!
  ...
  The span drawing routines of the software renderer handle the drawing of floor textures to the
  framebuffer. They use a number of global variables starting with "ds_" for parameter passing.
...
static void wipe_initMelt(int width, int height)
{
    ...
    wipe_shittyColMajorXform(
        (short *)wipe_scr_start, width * vid.BytesPerPixel / sizeof(short), height);
    wipe_shittyColMajorXform(
        (short *)wipe_scr_end, width * vid.BytesPerPixel / sizeof(short), height);
...
static wipe_t wipes[] = {{wipe_initColorXForm, wipe_doColorXForm, wipe_exitColorXForm},
