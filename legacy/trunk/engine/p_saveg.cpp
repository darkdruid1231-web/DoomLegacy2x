...
int acs_t::Marshal(LArchive &a)
{
    ...
    {
        a << temp;
        def = mp->ACS_FindScript(temp);
        a << temp;
        ip = reinterpret_cast<Sint32 *>(&mp->ACS_base[temp]);
...
int stair_t::Marshal(LArchive &a)
{
    ...
    a << state << resetcount << wait << stepdelay << speed << destheight << originalheight
...
int spritepres_t::Marshal(LArchive &a)
{
    ...
    {
        a << temp;
        info = aid[static_cast<mobjtype_t>(temp)];
        state_t *st;
        RE_PTR(st, states);
        spr = NULL;
        SetFrame(st);
        a << temp;
        animseq = static_cast<animseq_e>(temp);
    }
...
int DActor::Marshal(LArchive &a)
{
    ...
    if (a.IsStoring())
    {
        ...
        if (pos.z != floorz)
            diff |= MD_Z;
        if (vel != vec_t<fixed_t>(0, 0, 0))
            diff |= MD_MOM;

        if (mass != info->mass)
            diff |= MD_MASS;
        if (radius != info->radius)
            diff |= MD_RADIUS;
        if (height != info->height)
            diff |= MD_HEIGHT;

        if (health != info->spawnhealth)
            diff |= MD_HEALTH;
        if (flags != info->flags)
            diff |= MD_FLAGS;
        if (flags2 != info->flags2)
            diff |= MD_FLAGS2;
        if (eflags)
            diff |= MD_EFLAGS;
        ...
        if (special)
            diff |= MD_SPECIAL;
        if (reactiontime != info->reactiontime)
            diff |= MD_RTIME;
        if (floorclip != 0)
            diff |= MD_FLOORCLIP;
        if (team)
...
int PlayerPawn::Marshal(LArchive &a)
{
    ...
    else
    {
        ...
        a << temp;
        pendingweapon = weapontype_t(temp);
        a << temp;
        readyweapon = weapontype_t(temp);
...
int Map::Serialize(LArchive &a)
{
    ...
    //----------------------------------------------
    {
        ...
        // Perhaps we should really fake-setup the comparison map... or perhaps we'll just live with
        // it.
        ...
        mapsector_t *ms = comp.sectors =
            static_cast<mapsector_t *>(fc.CacheLumpNum(lumpnum + LUMP_SECTORS, PU_DAVE));
        sector_t *ss = sectors;
        ...
        doom_maplinedef_t *mld = comp.dlines =
            static_cast<doom_maplinedef_t *>(fc.CacheLumpNum(lumpnum + LUMP_LINEDEFS, PU_DAVE));
        hex_maplinedef_t *hld = comp.hlines;
        line_t *li = lines;

        mapsidedef_t *msd = comp.sides =
            static_cast<mapsidedef_t *>(fc.CacheLumpNum(lumpnum + LUMP_SIDEDEFS, PU_DAVE));
        side_t *si;
        ...
        for (i = 0; i < numlines; i++, li++)
        {
            ...
            if (li->sideptr[0])
            {
                ...
                if (si->toptexture && strncmp(si->toptexture->GetName(), msd[temp].toptexture, 8))
                    diff |= LD_S1TOPTEX;
                if (si->bottomtexture &&
                    strncmp(si->bottomtexture->GetName(), msd[temp].bottomtexture, 8))
                    diff |= LD_S1BOTTEX;
    ...
    for (i = 0; i < NumPolyobjs; i++)
        a << polyobjs[i].id << polyobjs[i].angle << polyobjs[i].origin.x << polyobjs[i].origin.y;
    ...
    // FS: levelscript contains the map global variables (everything else can be loaded from the
    // WAD)
...
int Map::Unserialize(LArchive &a)
{
    ...
    while (1)
    {
        ...
        if (diff & SD_FLOORHT)
            a << sectors[i].floorheight;
        if (diff & SD_CEILHT)
            a << sectors[i].ceilingheight;
        if (diff & SD_FLOORPIC)
        {
        ...
        if (diff2 & SD_FXOFFS)
            a << sectors[i].floor_xoffs;
        if (diff2 & SD_FYOFFS)
            a << sectors[i].floor_yoffs;
        if (diff2 & SD_CXOFFS)
            a << sectors[i].ceiling_xoffs;
        if (diff2 & SD_CYOFFS)
            a << sectors[i].ceiling_yoffs;

        if (diff2 & SD_STAIRLOCK)
            a << sectors[i].stairlock;
        else
