...
bool DActor::Touch(Actor *p)
{
    ...
    if (eflags & MFE_BLASTED && p->flags & MF_SHOOTABLE && p->flags & MF_MONSTER &&
        !(p->flags2 & MF2_BOSS))
    {
        ...
        else if (p->flags2 & MF2_REFLECTIVE)
        {
            ...
            switch (temp)
            {
                ...
                case MT_CENTAURLEADER:
                    if ((Abs(yaw - p->yaw) >> 24) > 45 ||
                        type == MT_HOLY_FX) // flank shot or wraithverge
                        goto explode;
    ...
    explode:
        ...
        if (owner)
        {
            ...
            if (ow && t)
                if (ow->type == t->type || (ow->type == MT_KNIGHT && t->type == MT_BRUISER) ||
        ...
        // damage / explode
        damage = ((P_Random() % 8) + 1) * damage;
        if (damage && p->Damage(this, owner, damage, dtype) && !(p->flags & MF_NOBLOOD))
        {
            /* Hexen:
            if (P_Random() < 192)
...
bool Actor::Damage(Actor *inflictor, Actor *source, int damage, int dtype)
{
        ...
        !(inflictor->flags2 & MF2_NODMGTHRUST))
    {
        ...
        if (damage < 40 && damage > health && (pos.z - inflictor->pos.z) > 64 && (P_Random() & 1))
        {
            ang += ANG180;
            thrust *= 4;
        }
...
bool DActor::Damage(Actor *inflictor, Actor *source, int damage, int dtype)
{
    ...
    */
    ...
    if ((P_Random() < info->painchance) && !(eflags & MFE_SKULLFLY || flags & MF_CORPSE))
    {
    ...
    // get angry
    if ((!threshold || (flags2 & MF2_QUICKTORETALIATE)) && source && source != target)
    {
        if (source->team == team && (cv_infighting.value == 0 || source->flags2 & MF2_NOTARGET))
            return true;
        ...
        switch (d->type)
        {
            ...
            case MT_SHARDFX1:
              ...
              case MT_CSTAFF_MISSILE:
                ...
                P_PoisonPlayer(player, source, 20); // only for ppawns (dactors get full damage
              instead (all poison)) damage >>= 1; break; case MT_POISONDART: P_PoisonPlayer(player,
              source, 20); damage >>= 1; break; case MT_POISONCLOUD: if (player->poisoncount < 4)
                  {
                    P_PoisonDamage(player, source, 15+(P_Random()&15), false); // Don't play
              painsound P_PoisonPlayer(player, source, 50); S_StartSound(target,
    ...
    if (!(flags & MF_CORPSE))
    {
        ...
        if (save > 0)
        {
            ...
            for (i = armor_armor; i < NUMARMOR; i++)
                ...
                {
                    armorpoints[i] -=
                        int(damage * ArmorIncrement[pclass][i] / (100 * armorfactor[i]));
                    if (armorpoints[i] <= 2)
                        armorpoints[i] = 0;
                }
