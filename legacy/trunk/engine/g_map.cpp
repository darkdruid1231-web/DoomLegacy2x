...
Actor *Map::SpawnActor(Actor *a, fixed_t spawnheight)
{
    ...
    position_check_t *ccc =
        a->CheckPosition(a->pos, Actor::PC_LINES); // for missiles owner is not yet set => collides,
                                                   // so we only check geometry
...
{
    // NOTE: nz may have the values ONFLOORZ, ONCEILINGZ and FLOATRANDZ in addition to real z
    // coords...
...
void Map::SpawnPlayer(PlayerInfo *pi, mapthing_t *mthing)
{
    ...
    p->TeleportMove(p->pos); // teleport to current location to telefrag bothersome obstacles (read:
                             // other players)
                ...
                switch (p->playerstate)
                {
                    case PST_LEAVINGMAP:
                        ...
                        if (game.server)
                        {
                            ...
                            MapInfo *next =
                                game.FindMapInfo(p->requestmap); // do we need an intermission?
                            ...
                            if (next && cv_intermission.value)
                            {
                                ...
                                p->playerstate =
                                    PST_INTERMISSION; // c2sIntermissionDone rpc resets this
                                return true;
                            }
                            else
                                p->playerstate = PST_NEEDMAP;
...
void Map::BossDeath(const DActor *mo)
{
    ...
    for (i = 0; i < n; i++)
        if ((players[i]->playerstate == PST_INMAP || players[i]->playerstate == PST_FINISHEDMAP) &&
    ...
    switch (mo->type)
    {
        ...
        case MT_CYBORG:
            ...
            else
            {
                ...
                EV_DoDoor(
                    666, NULL, NULL, vdoor_t::Open | vdoor_t::Blazing, 4 * VDOORSPEED, VDOORWAIT);
                return;
            }
...
Actor *ActorInfo::Spawn(Map *m, mapthing_t *mt, bool initial) const
{
    ...
    if (flags2 & MF2_FLOATBOB)
    {
        ...
        p->special1 =
            (p->pos.z - p->floorz).floor(); // floating height above floor (integer, NOT fixed_t)
    }
    ...
    if (initial)
    {
        // NOTE: To make Hexen maps with TID counting scripts playable, respawned monsters get no
        // TID.
        p->tid = mt->tid;
        if (p->tid)
    ...
    else
    {
        ...
        Actor *fog =
            m->SpawnDActor(nx, ny, p->pos.z + (game >= gm_heretic ? TELEFOGHEIGHT : 0), MT_IFOG);
