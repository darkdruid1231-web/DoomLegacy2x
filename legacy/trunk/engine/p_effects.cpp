...
DActor *Map::SpawnSplash(const vec_t<fixed_t> &pos,
                         fixed_t z,
                         int sound,
                         mobjtype_t base,
                         mobjtype_t chunk,
...
void Map::SpawnSmoke(const vec_t<fixed_t> &r)
{
    ...
    DActor *th =
        SpawnDActor(r + vec_t<fixed_t>(8 * Random() - 4, 8 * Random() - 4, 3 * Random()), MT_SMOK);
    th->vel.z = 1;
    th->tics -= P_Random() & 3;
...
MapEffect::MapEffect(Map *m)
{
    ...
    {
        if (s[i].SkyCeiling() || s[i].special == SS_IndoorLightning1 ||
