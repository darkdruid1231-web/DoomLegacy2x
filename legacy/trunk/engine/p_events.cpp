...
//-----------------------------------------------------------------------------
//
// $Id: p_events.cpp 522 2008-01-20 22:42:23Z smite-meister $
//
...
bool Map::ActivateLine(line_t *line, Actor *thing, int side, int atype)
{
    ...
    if (!p)
    {
        ...
        bool monster_ok = (!(act <= SPAC_USE || act == SPAC_PASSUSE) ||
                           (line->flags & ML_MONSTERS_CAN_ACTIVATE)) &&
...
// Hexen linedefs
//
static inline float SPEED(byte a)
{
    return float(a) / 8.0f;
}
static inline fixed_t HEIGHT(byte a)
{
    return fixed_t(a);
}
static inline int TICS(byte a)
{
    return (a * TICRATE) / 35;
}
static inline int OCTICS(byte a)
{
    return (a * TICRATE) / 8;
}
static inline angle_t ANGLE(byte a)
{
    return angle_t(a) << 24;
} // angle_t is Uint32, Hexen angle is a 8-bit byte
...
bool Map::ExecuteLineSpecial(unsigned special, byte *args, line_t *line, int side, Actor *mo)
{
    ...
    switch (special)
    {
        ...
        case 2: // Poly Rotate Left
            success = EV_ActivatePolyobj(args[0],
                                         polyobject_t::po_rotate,
                                         float(ANGLE(args[1])) / 8.0f,
                                         ANGLE(args[2]),
                                         0,
                                         0,
                                         false);
        ...
        case 3: // Poly Rotate Right
            ...
            break;
        case 4: // Poly Move
            success = EV_ActivatePolyobj(
                args[0], polyobject_t::po_move, SPEED(args[1]), ANGLE(args[2]), args[3], 0, false);
            break;
        case 5: // Poly Explicit Line:  Only used in initialization
            break;
        case 6: // Poly Move Times 8
            success = EV_ActivatePolyobj(args[0],
                                         polyobject_t::po_move,
                                         SPEED(args[1]),
                                         ANGLE(args[2]),
                                         8 * args[3],
                                         0,
        ...
        case 20: // Floor Lower by Value
            success =
                EV_DoFloor(tag, line, floor_t::RelHeight, -SPEED(args[1]), 0, -HEIGHT(args[2]));
            break;
        ...
        case 26: // Stairs Build Down Normal
            success = EV_BuildHexenStairs(
                tag, stair_t::Normal, -SPEED(args[1]), -HEIGHT(args[2]), args[4], args[3]);
            break;
        ...
        case 29: // Build Pillar (no crushing)
            success =
                EV_DoElevator(tag, elevator_t::ClosePillar, SPEED(args[1]), HEIGHT(args[2]), 0, 0);
            break;
        case 30: // Open Pillar
            success = EV_DoElevator(
                tag, elevator_t::OpenPillar, SPEED(args[1]), HEIGHT(args[2]), HEIGHT(args[3]), 0);
            break;
        case 31: // Stairs Build Down Sync
            success = EV_BuildHexenStairs(
                tag, stair_t::Sync, -SPEED(args[1]), -HEIGHT(args[2]), args[3], 0);
            break;
        ...
        case 35: // Raise Floor by Value Times 8
            success =
                EV_DoFloor(tag, line, floor_t::RelHeight, SPEED(args[1]), 0, 8 * HEIGHT(args[2]));
            break;
        ...
        case 40: // Ceiling Lower by Value
            success =
                EV_DoCeiling(tag, line, ceiling_t::RelHeight, -SPEED(args[1]), 0, -HEIGHT(args[2]));
            break;
        ...
        case 42: // Ceiling Crush and Raise
            success = EV_DoCrusher(tag,
                                   args[4] ? ceiling_t::Silent : 0,
                                   SPEED(args[1]),
                                   args[3] ? SPEED(args[3]) : SPEED(args[1]) / 2,
                                   args[2],
                                   HEIGHT(8));
            break;
        case 43: // Ceiling Lower and Crush
            success =
                EV_DoCeiling(tag, line, ceiling_t::Floor, -SPEED(args[1]), args[2], HEIGHT(8));
            break;
        case 44: // Ceiling Crush Stop
            success = EV_StopCeiling(tag);
            break;
        case 45: // Ceiling Crush Raise and Stay
            success = EV_DoCrusher(
                tag, crusher_t::CrushOnce, SPEED(args[1]), SPEED(args[1]) / 2, args[2], HEIGHT(8));
            break;
            /*
        ...
        case 63: // Plat Down-by-Value*8-Wait-Up-Stay
            success = EV_DoPlat(
                tag, line, plat_t::RelHeight, SPEED(args[1]), TICS(args[2]), -8 * HEIGHT(args[3]));
            break;
        ...
        case 92: // Poly Move Override
            success = EV_ActivatePolyobj(
                args[0], polyobject_t::po_move, SPEED(args[1]), ANGLE(args[2]), args[3], 0, true);
            break;
        ...
        case 94: // Build Pillar Crush
            success = EV_DoElevator(
                tag, elevator_t::ClosePillar, SPEED(args[1]), HEIGHT(args[2]), 0, args[3]);
            break;
        ...
        case 135: // Thing_Spawn
            success =
                EV_ThingSpawn(args[0], aid[TranslateThingType[args[1]]], ANGLE(args[2]), true);
            break;
        ...
        case 137: // Thing_SpawnNoFog
            success =
                EV_ThingSpawn(args[0], aid[TranslateThingType[args[1]]], ANGLE(args[2]), false);
            break;
        case 138: // Floor_Waggle
            success = EV_DoFloorWaggle(tag,
                                       HEIGHT(args[1]) >> 3,
                                       angle_t(args[2] << 20),
                                       ANGLE(args[3]) << 2,
                                       args[4] * 35);
        ...
        case 201: // ZDoom Generic_Ceiling
        {
            ...
            success = EV_DoCeiling(tag,
                                   line,
                                   ZDoom_GenCeiling(args[3], args[4]),
                                   args[4] & 0x08 ? SPEED(args[1]) : -SPEED(args[1]),
                                   (args[4] & 0x10) ? 20 : 0,
                                   neg_height ? -HEIGHT(args[2]) : HEIGHT(args[2]));
        }
        ...
        case 203: // ZDoom Generic_Lift
            success = EV_DoPlat(
                tag, line, ZDoom_GenLift(args[3]), SPEED(args[1]), TICS(args[2]), HEIGHT(args[4]));
            break;
        ...
        case 246: // ZDoom Elevator_MoveToFloor
            if (line)
                success = EV_DoElevator(
                    tag, elevator_t::Current, SPEED(args[1]), line->frontsector->floorheight);
            else
                success = false;
            break;
        ...
        case LINE_LEGACY_EXT: // Legacy extensions to Hexen linedef namespace (all under this one
                              // type)
            switch (args[0])
            {
                case LINE_LEGACY_FS:
