...
//-----------------------------------------------------------------------------
//
// $Id: p_spec.cpp 522 2008-01-20 22:42:23Z smite-meister $
//
...
{
    m->AddThinker(
        this); // when a sectoreffect is constructed, it is immediately added to the Thinker list.
    sector = s;
}
...
fixed_t sector_t::FindNextLowestFloor(fixed_t currentheight)
{
    ...
    for (int i = 0; i < linecount; i++)
        if ((other = getNextSector(lines[i], this)) && other->floorheight < currentheight)
        {
            ...
            while (++i < linecount)
                if ((other = getNextSector(lines[i], this)) && other->floorheight > h &&
...
fixed_t sector_t::FindNextLowestCeiling(fixed_t currentheight)
{
    ...
    for (int i = 0; i < linecount; i++)
        if ((other = getNextSector(lines[i], this)) && other->ceilingheight < currentheight)
        {
            ...
            while (++i < linecount)
                if ((other = getNextSector(lines[i], this)) && other->ceilingheight > h &&
...
sector_t *sector_t::FindModelFloorSector(fixed_t floordestheight)
{
    ...
    for (int i = 0; i < (!boomsupport && sec->linecount < linecount ? sec->linecount : linecount);
         i++)
    {
        if (twoSided(i))
        {
...
{
    int index = (*start >= 0) ? lines[*start].nextid : lines[lineid % (unsigned)numlines].firstid;
...
bool Map::TagBusy(unsigned tag)
{
    ...
    {
        if (sectors[i].floordata || sectors[i].ceilingdata || sectors[i].lightingdata)
            return true;
    }
    return false;
}
...
int Map::SpawnSectorSpecial(int sp, sector_t *sec)
{
        ...
        else if (temp >= 201 && temp <= 224)
        {
            ...
            switch (temp)
            {
                ...
                case SS_LightSequence_2:
                    // Phased light sequencing. Leave them be, they fit into the unused lightmask
                    // area.
    ...
    if (game.mode == gm_heretic)
    {
        ...
        if (temp < 20)
            switch (temp)
            {
                ...
                case HERETIC_Lava_FlowEast:
                    AddThinker(new scroll_t(
                        scroll_t::sc_carry_floor, 28.0f / 32.0f, 0, NULL, sec - sectors, false));
                    // fallthru
        ...
        else if (temp <= 39)
        {
            ...
            AddThinker(new scroll_t(
                scroll_t::sc_floor | scroll_t::sc_carry_floor, dx, dy, NULL, sec - sectors, false));
        }
        else if (temp <= 51)
        {
            // Heretic winds
            ...
            AddThinker(new scroll_t(
                scroll_t::sc_carry_floor | scroll_t::sc_wind, dx, dy, NULL, sec - sectors, false));
        }
    }
    else
    {
...
void Map::SpawnLineSpecials()
{
    ...
    for (int i = 0; i < numlines; i++)
    {
        ...
        // Legacy extensions are mapped to Hexen linedef namespace so they are reachable from Hexen
        // as well! only check for clearable stuff here
        ...
        switch (special)
        {
            ...
            case 100: // Scroll_Texture_Left
                AddThinker(new scroll_t(scroll_t::sc_side,
                                        fixed_t(l->args[0]) >> 6,
                                        0,
                                        NULL,
                                        l->sideptr[0] - sides,
            ...
            case 224: // Scroll_Ceiling
            {
                ...
                /*
                  ...
                  SpawnScroller(l->args[3]-128, l->args[4]-128, tag, scroll_t::sc_ceiling,
                l->args[1] & 0x3);
                */
            }
            break;
...
void scroll_t::Think()
{
    ...
    if (type & sc_carry_floor)
    {
        ...
        fixed_t waterheight =
            sec->heightsec != -1 && mp->sectors[sec->heightsec].floorheight > height
                ? mp->sectors[sec->heightsec].floorheight
                : fixed_t::FMIN;
...
void Map::SpawnScroller(line_t *l, unsigned tag, int type, int control)
{
    ...
    if (type == scroll_t::sc_side)
    {
        ...
        {
            AddThinker(new scroll_t(
                scroll_t::sc_side, -sides[s].textureoffset, sides[s].rowoffset, NULL, s, accel));
        }
        else
            for (s = -1; (l2 = FindLineFromID(tag, &s)) !=
                         NULL;) // NOTE: "tag" actually a line id, only done with ZDoom type 222
                if (l2 != l)
                {
                    ...
                    AddThinker(
                        new scroll_t(scroll_t::sc_side, x, y, csec, l2->sideptr[0] - sides, accel));
                }
        return;
    }
