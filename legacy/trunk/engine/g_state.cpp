...
int GameInfo::GetFrags(fragsort_t **fragtab, int type)
{
    ...
    if (cv_teamplay.value)
    {
        ...
        // const int cm = m;
        std::vector<std::vector<int>> teamfrags(m, std::vector<int>(m));
        ...
        for (u = Players.begin(); u != Players.end(); u++)
        {
            ...
            for (w = Players.begin(); w != Players.end(); w++)
            {
                ...
                teamfrags[team1][team2] += (*u).second->Frags[(*w).first];
            }
        }
            ...
            case 1: // buchholtz
                for (i = 0, u = Players.begin(); u != Players.end(); i++, u++)
                {
                    ...
                    for (w = Players.begin(); w != Players.end(); w++)
                        ...
                        {
                            int k = (*w).first;
                            ft[i].count += (*u).second->Frags[k] *
                                           ((*w).second->score + (*w).second->Frags[k]);
...
void GameInfo::Ticker()
{
    ...
    for (player_iter_t t = Players.begin(); t != Players.end();)
    {
        ...
        if (p->playerstate == PST_REMOVE)
        {
            ...
            if (!p->mp)
                RemovePlayer(p->number); // first the maps throw out the removed player, then the
                                         // game proper.
