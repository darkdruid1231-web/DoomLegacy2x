...
//-----------------------------------------------------------------------------
//
// $Id: am_map.cpp 527 2008-01-27 02:25:14Z smite-meister $
//
...
#define R 1.0f
mline_t triangle_guy[] = {{{-.867f * R, -.5f * R}, {.867f * R, -.5f * R}},
                          {{.867f * R, -.5f * R}, {0, R}},
                          {{0, R}, {-.867f * R, -.5f * R}}};
#undef R
...
#define R 1.0f
mline_t thintriangle_guy[] = {{{-.5f * R, -.7f * R}, {R, 0}},
                              {{R, 0}, {-.5f * R, .7f * R}},
                              {{-.5f * R, .7f * R}, {-.5f * R, -.7f * R}}};
#undef R
...
inline fixed_t FTOM(int x)
{
    return x * scale_ftom;
}
inline int MTOF(fixed_t x)
{
    return (x * scale_mtof).floor();
}
...
bool AutoMap::Responder(event_t *ev)
{
    ...
    if (!active)
    {
        ...
        {
            // faB: prevent alt-tab in win32 version to activate automap just before minimizing the
            // app
...
void AutoMap::drawWalls()
{
    ...
    for (i = 0; i < mp->numlines; i++)
    {
        ...
        {
            if (!(mp->lines[i].flags & ML_DONTDRAW))
        ...
        if (p == mpawn)
        {
            if (am_cheating)
                AM_drawLineCharacter(
                    cheat_player_arrow, NUMCHEATPLYRLINES, 0, p->yaw, DWHITE, p->pos.x, p->pos.y);
            else
                AM_drawLineCharacter(
                    player_arrow, NUMPLYRLINES, 0, p->yaw, DWHITE, p->pos.x, p->pos.y);
        }
        else
        {
            int color;
            ...
            AM_drawLineCharacter(player_arrow, NUMPLYRLINES, 0, p->yaw, color, p->pos.x, p->pos.y);
        }
    }
}
