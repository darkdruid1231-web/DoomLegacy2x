...
static const char *StandardLabels[] = {
    "spawn", "see", "melee", "missile", "pain", "death", "xdeath", "crash", "raise"};
...
void ActorInfo::AddStates(const char *spr, const char *frames, int tics, const char *func)
{
    ...
    {
        dactor_mnemonic_t *m;
        for (m = BEX_DActorMnemonics; m->name && strcasecmp(&func[2], m->name); m++)
            ; // ignore the initial "A_"
        if (!m->name)
...
void ActorInfo::FinishSequence(const char *jumplab, int offset)
{
    ...
    {
        // loop to first state in sequence (equivalent to "goto current_label", but save some effort
        // here)
...
bool ActorInfo::UpdateSequences()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (s->jumplabelnum != -2)
            continue; // already ok (never changes even if new labels are added or old ones
                      // redefined!)
        ...
        {
            s->jumplabelnum =
                s - &labels.front(); // TODO is this certain to work? would iterators be better?
        }
...
#define MAX_REDIRECTS 10 // allow max. 10 redirects
            ...
            if (s->num_states) // for redirects, do nothing
            {
                ...
                if (s->jumpoffset < labels[j].num_states)
                    s->label_states[s->num_states - 1].nextstate =
                        &labels[j].label_states[s->jumpoffset];
                else
                    I_Error(
                        "DECORATE: State offset too large.\n"); // TODO wrap offsets to next seqs?
            }
        }
    }
...
void ActorInfo::PrintDECORATEclass()
{
    ...
    if (spawnstate)
      {
        printf("  Spawn:\n");
        printf("    %s %s %d %s\n", spritenames[s->sprite], temp, s->tics,
    BEX_DActorMnemonics[i].name);
      }
    printf("  }\n");
    */
    printf("}\n\n");
...
{
  public:
    SkyboxCameraAI(const string &n, int en) : ActorInfo(n, en)
    {
        spawn_always = true;
...
/// Convert static game data structures into dynamic ones. DECORATE scripts are processed after the
/// conversion.
