...
// same for Heretic
static point_t HereticMapSpots[3][NUMMAPS] = {{{172, 78},
                                               {86, 90},
                                               {73, 66},
                                               {159, 95},
                                               {148, 126},
...
static void WI_drawLF(const char *name)
{
    ...
    if (finished_tex)
        ...
        big_font->DrawString(
            (BASEVIDWIDTH - big_font->StringWidth("Finished")) / 2, y, "Finished", FB);
}
...
static void WI_drawEL(const char *nextname)
{
    ...
    {
        big_font->DrawString(
            (BASEVIDWIDTH - big_font->StringWidth("Entering")) / 2, y, "Entering", FB);
        y += 5 * big_font->Height() / 4;
    }
...
void Intermission::DrawCoopStats()
{
    ...
    else
    {
        ...
        if (big_font)
        {
            // use FontB if any
            big_font->DrawString(
                NG_STATSX + NG_SPACINGX - big_font->StringWidth("Kills"), NG_STATSY, "Kills", FB);
            big_font->DrawString(NG_STATSX + 2 * NG_SPACINGX - big_font->StringWidth("Items"),
                                 NG_STATSY,
                                 "Items",
                                 FB);
            big_font->DrawString(
                NG_STATSX + 3 * NG_SPACINGX - big_font->StringWidth("Scrt"), NG_STATSY, "Scrt", FB);
            if (dofrags)
                big_font->DrawString(NG_STATSX + 4 * NG_SPACINGX - big_font->StringWidth("Frgs"),
                                     NG_STATSY,
                                     "Frgs",
                                     FB);
...
void Intermission::Start(
    const MapInfo *f, const MapInfo *n, int maptic, int kills, int items, int secrets)
{
    if (!n)
