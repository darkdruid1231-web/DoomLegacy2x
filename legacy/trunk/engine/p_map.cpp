...
//-----------------------------------------------------------------------------
//
// $Id: p_map.cpp 531 2008-11-02 09:03:56Z smite-meister $
//
...
extern const fixed_t MAXSTEP = 24; ///< Max Z move up or down without jumping. Above this, a height
                                   ///< difference is considered a 'dropoff'.
const fixed_t MAXWATERSTEP = 37;   ///< Same, but in water.

/*!
  Attempt to move to a new position, crossing special lines in the way.
  \return An std::pair where first member is true if move succeeded, and the second a
  position_check_t* for the new position.
 */
pair<bool, position_check_t *> Actor::TryMove(fixed_t nx, fixed_t ny, bool allowdropoff)
{
    ...
    // No traces are used, rather blockmapcells are iterated one by one by PIT_CheckLine, and we
    // stop when the first blocking thing/line is found => 1) there might be others we actually hit
    // first. If the iteration is stopped by a line we have immediate collision damage and impact
    // activation. z fit is only checked later, possible z-blocking lines are pushed to spechit.
    ...
    if (!ccc->xy_move_ok)
    {
        ...
        if (ccc->block_thing)
        {
            ...
            if (ccc->block_thing->Top() - Feet() <= MAXSTEP &&
                // ccc->block_thing->subsector->sector->ceilingheight - ccc->block_thing->Top() >=
                // height &&
    ...
    if (!(flags & MF_NOCLIPLINE))
    {
        ...
        if (!allowdropoff && !(flags & MF_DROPOFF) && !(eflags & MFE_BLASTED))
            if (ccc->op.Drop() > MAXSTEP && !floor_thing)
                return pair<bool, position_check_t *>(
                    false, ccc); // don't go over a dropoff (unless blasted)
    ...
    if (!(flags & MF_NOCLIPLINE))
    {
        while (ccc->spechit.size())
        {
            ...
            {
                if (flags2 & MF2_MCROSS && mp->ActivateLine(ld, this, oldside, SPAC_MCROSS))
                    continue;

                if (flags2 & MF2_PCROSS && mp->ActivateLine(ld, this, oldside, SPAC_PCROSS))
                    continue;
...
void Actor::SetPosition()
{
    ...
    if (!(flags & MF_NOSECTOR))
    {
        ...
        if (sprev != NULL || snext != NULL)
            I_Error("Actor::SetPosition: thing at (%d, %d) is already linked",
                    pos.x.floor(),
                    pos.y.floor());
#endif
...
/// Data for PIT_CheckThing and PIT_CheckLine, also holds the return value of last call to
/// Actor::CheckPosition()
...
static bool PIT_CheckLine(line_t *ld)
{
    ...
    if (!ld->backsector || // one-sided line
        (!(tmthing->flags &
           MF_MISSILE) && // missile and Camera can cross uncrossable lines with a backsector
...
position_check_t *Actor::CheckPosition(const vec_t<fixed_t> &p, poscheck_e mode)
{
    ...
    if (mode & Actor::PC_LINES)
    {
        ...
        // check lines
        if (!(flags & MF_NOCLIPLINE) &&
            !mp->blockmap->IterateLinesRadius(p.x, p.y, radius, PIT_CheckLine))
            ret = false;
    }
    ...
    if (mode & Actor::PC_THINGS)
    {
        ...
        // based on their origin point, and can overlap into adjacent blocks by up to MAXRADIUS
        // units.
        if (ret && // early out
            !(flags & MF_NOCLIPTHING) &&
            !mp->blockmap->IterateThingsRadius(p.x, p.y, radius + MAXRADIUS, PIT_CheckThing))
            ret = false;
    }
...
bool Actor::TestLocation(const vec_t<fixed_t> &p)
{
    ...
    return ccc->xy_move_ok && (p.z >= ccc->op.bottom) && (p.z + height <= ccc->op.top);
}
...
static bool PTR_SlideTraverse(intercept_t *in)
{
    ...
    else if (!(li->flags & ML_BLOCKING))
    {
        ...
        if (!(open->Range() < slidemo->height || open->top < slidemo->Top() ||
...
static void P_HitSlideLine(line_t *ld)
{
    ...
    vec_t<float> line_normal(
        ld->dy.Float(), -ld->dx.Float(), 0); // line rotated 90 degrees clockwise
...
static bool PTR_BounceTraverse(intercept_t *in)
{
    ...
    else
    {
        ...
        // will it fit through?
        if (open->top >= slidemo->Top() && open->bottom <= slidemo->Feet())
            return true; // this line doesn't block movement
    }
    ...
    if (in->isaline)
    {
        ...
        if (li->frontsector->ffloors || li->backsector->ffloors)
        {
            ...
            for (ffloor_t *rover = li->frontsector->ffloors; rover; rover = rover->next)
            {
                ...
                if (trace.lastz > *rover->topheight && trace.sin_pitch < 0 &&
                    trace.sin_pitch < highsine)
                    frontflag |= 0x2;

                if (trace.lastz < *rover->bottomheight && trace.sin_pitch > 0 &&
                    trace.sin_pitch > lowsine)
                    frontflag |= 0x2;
            }
        ...
        trace.lastz = trace.start.z +
                      fixed_t(trace.sin_pitch * dist); // TODO trace.sin_pitch is not well-defined
...
Actor *Actor::AimLineAttack(angle_t ang, float distance, float &sinpitch)
{
    ...
    // Since monsters shouldn't change their "pitch" angle, why not use the same routine for them
    // also?
...
static bool PTR_LineTrace(intercept_t *in)
{
        ...
        if (!trace.HitZPlane(front))
        {
            ...
            if (li->special && interact)
                m->ActivateLine(
                    ...
                    shootthing,
                    0,
                    SPAC_IMPACT); // your documentation no longer confuses me, old version
    ...
    else
    {
        ...
        // an Actor can be in several sectors at once, so we need the exact impact point and its
        // sector
...
Actor *Actor::LineAttack(angle_t ang, float distance, float sine, int damage, int dtype)
{
    ...
    else if (target_actor)
    {
        ...
        // check for physical attacks on a ghost FIXME this is stupid. We should use some sort of
        // damage type system. (if damagetype != dt_ethereal ...)
        /*
      ...
      return true;
        */
        ...
        if (!(target_actor->flags & MF_NOBLOOD || target_actor->flags2 & MF2_INVULNERABLE))
        {
...
static bool PTR_BloodTraverse(intercept_t *in)
{
    if (in->isaline)
    {
        ...
        li->AddWallSplat((game.mode >= gm_heretic) ? "BLODC0" : "BLUDC0",
                         P_PointOnLineSide(blood_x, blood_y, li),
                         z,
                         frac,
                         SPLATDRAWMODE_TRANS);
        return false;
...
void Map::SpawnBloodSplats(const vec_t<fixed_t> &r, int damage, fixed_t px, fixed_t py)
{
    ...
    R_AddFloorSplat(
        bloodthing->subsector, "STEP2", r.x, r.y, bloodthing->floorz, SPLATDRAWMODE_SHADE);
#endif
}
...
bool PlayerPawn::UsePuzzleItem(int type)
{
    ...
    mp->blockmap->PathTraverse(
        pos, pos + delta, PT_ADDLINES | PT_ADDTHINGS, PTR_PuzzleItemTraverse);
...
static bool PIT_ChangeSector(Actor *thing)
{
    ...
    if (crushdamage && !(thing->mp->maptic % 4))
    {
            ...
            !(thing->flags2 & MF2_INVULNERABLE))
        {
            ...
            DActor *mo =
                thing->mp->SpawnDActor(thing->pos.x, thing->pos.y, thing->Center(), MT_BLOOD);
...
static bool PIT_ThrustStompThing(Actor *thing)
{
    ...
    if (abs(thing->pos.x - tmthing->pos.x) >= blockdist ||
        abs(thing->pos.y - tmthing->pos.y) >= blockdist || thing->Feet() > tmthing->Top())
        return true; // didn't hit it
...
void P_ThrustSpike(Actor *actor)
{
    ...
    actor->mp->blockmap->IterateThingsRadius(
        actor->pos.x, actor->pos.y, actor->radius + MAXRADIUS, PIT_ThrustStompThing);
}
