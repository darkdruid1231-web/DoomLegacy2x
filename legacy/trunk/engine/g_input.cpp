...
consvar_t cv_usemouse[NUM_MICE] = {
    {"use_mouse", "1", CV_SAVE | CV_CALL, usemouse_cons_t, I_StartupMouse},
...
// fixed_t MaxPlayerMove[NUMCLASSES] = { 60, 50, 45, 49 }; // implemented as pawn speeds in
// info_m.cpp Otherwise OK, but fighter should also run sideways almost as fast as forward, (59/60)
// instead of (80/100). Weird. Affects straferunning.
...
#define SLOWTURNTICS 6
    ...
    bool mouseaiming =
        (cks < NUM_MICE) ? ControlDown(gc[gc_mouseaiming]) ^ cv_automlook[cks].value : false;
    ...
    if (pawn)
    {
        ...
        else if (ControlDown(gc[gc_prevweapon]))
        {
            ...
            for (int i = gc_weapon1; i <= gc_weapon8; i++)
...
bool G_MapEventsToControls(event_t *ev)
{
    if (game.inventory)
        for (unsigned i = 0; i < NUM_LOCALHUMANS; i++)
            if (LocalPlayers[i].info && LocalPlayers[i].info->InventoryResponder(
                                            gamecontrol[LocalPlayers[i].controlkeyset], ev))
                return true;
    ...
    switch (ev->type)
    {
        ...
        case ev_mouse: // buttons are virtual keys
            mousex[0] +=
                int(ev->data2 * ((cv_mousesensx[0].value * cv_mousesensx[0].value) / 110.0f + 0.1));
            mousey[0] +=
                int(ev->data3 * ((cv_mousesensy[0].value * cv_mousesensy[0].value) / 110.0f + 0.1));
            break;

        case ev_mouse2: // buttons are virtual keys
            mousex[1] +=
                int(ev->data2 * ((cv_mousesensx[1].value * cv_mousesensx[1].value) / 110.0f + 0.1));
            mousey[1] +=
                int(ev->data3 * ((cv_mousesensy[1].value * cv_mousesensy[1].value) / 110.0f + 0.1));
            break;
...
const char *gamecontrolname[num_gamecontrols] = {
    ...
    "forward",    "backward",   "strafe",     "straferight",   "strafeleft",    "speed",
    "turnleft",   "turnright",  "lookup",     "lookdown",      "centerview",    "mouseaiming",
    "fire",       "use",        "jump",       "flydown",       "weapon1",       "weapon2",
    "weapon3",    "weapon4",    "weapon5",    "weapon6",       "weapon7",       "weapon8",
    "nextweapon", "prevweapon", "bestweapon", "inventorynext", "inventoryprev", "inventoryuse"};
...
void G_Controldefault()
{
    ...
    commoncontrols[gk_console][0] =
        KEY_CONSOLE; // This works out-of-the-box in US keyboads, not in others.
