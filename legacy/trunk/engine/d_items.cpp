...
//-----------------------------------------------------------------------------
//
// $Id: d_items.cpp 447 2007-04-06 15:31:11Z smite-meister $
//
...
weaponinfo_t wpnlev1info[NUMWEAPONS] = {
    ...
    {am_shell, 2, S_DSGUNUP, S_DSGUNDOWN, S_DSGUN, S_DSGUN1, S_DSGUN1, S_DSGUNFLASH1}, // super
                                                                                       // shotgun
    ...
    {am_misl,
     ...
     S_MISSILE1,
     S_MISSILE1,
     S_MISSILEFLASH1}, // missile launcher
    {am_cell, 1, S_PLASMAUP, S_PLASMADOWN, S_PLASMA, S_PLASMA1, S_PLASMA1, S_PLASMAFLASH1}, // plasma
                                                                                            // rifle
    ...
    {am_noammo,
     ...
     S_MWANDATK_1,
     S_WNULL}, // Mage - Wand
    {am_noammo, 2, S_FAXEUP, S_FAXEDOWN, S_FAXEREADY, S_FAXEATK_1, S_FAXEATK_1, S_WNULL}, // Fighter
                                                                                          // - Axe
    {am_mana1,
     1,
     S_CSTAFFUP,
    ...
    {am_manaboth,
     ...
     S_MSTAFFATK_1,
     S_WNULL}, // Mage - Staff
    {am_noammo, 0, S_SNOUTUP, S_SNOUTDOWN, S_SNOUTREADY, S_SNOUTATK1, S_SNOUTATK1, S_WNULL} // Pig -
                                                                                            // Snout
};

// right now Doom and Hexen weapons are exactly the same when tome of power is active, but who
// knows...
...
void P_SetDormantArtifact(DActor *arti)
{
    ...
    if (cv_deathmatch.value && !(arti->flags & MF_DROPPED))
    {
        ...
        switch (arti->type)
        {
            ...
            default:
                arti->SetState(S_DORMANTARTI1_1); // 40 s. Identical to the Heretic state sequence
                                                  // S_DORMANTARTI1.
                break;
        }
    }
        ...
        arti->SetState(
            S_XDEADARTI1); // Don't respawn at all. Identical to Heretic sequence S_DEADARTI1.
}
...
void PlayerPawn::TouchSpecialThing(DActor *thing)
{
    ...
    if (script)
    {
        ...
        mp->FS_RunScript(
            script, this); // too bad FS can't use parameters, maybe we should use ACS instead...
    }
    else
        switch (stype)
        {
