...
void MsgBox::Draw()
{
    ...
    for (p = text; *p; p++)
    {
        if (*p == '\n')
        {
            ...
            *p = '\0';
            hud_font->DrawString(
                (BASEVIDWIDTH - slength * hud_font->Width()) / 2, ytemp, s, V_SCALE);
...
int quitsounds[8] = {
    sfx_pldeth, sfx_dmpain, sfx_popain, sfx_gib, sfx_teleport, sfx_posit1, sfx_posit3, sfx_sgtatk};

int quitsounds2[8] = {
    sfx_vilact, sfx_powerup, sfx_boscub, sfx_gib, sfx_skeswg, sfx_kntdth, sfx_bspact, sfx_sgtatk};
...
static menuitem_t SinglePlayer_MI[] = {menuitem_t(IT_ACT, "M_NGAME", "NEW GAME", M_NewGame, 'n'),
...
static menuitem_t Class_MI[] = {menuitem_t(IT_ACT, NULL, "FIGHTER", M_Class, 'f'),
                                menuitem_t(IT_ACT, NULL, "CLERIC", M_Class, 'c'),
                                menuitem_t(IT_ACT, NULL, "MAGE", M_Class, 'm')};

Menu
    ClassDef(NULL, "Choose class:", &SinglePlayerDef, ITEMS(Class_MI), 66, 66, 0, &Menu::DrawClass);
...
menuitem_t Skill_MI[] = {menuitem_t(IT_ACT, "M_JKILL", "I'm too young to die.", M_ChooseSkill, 'i'),
...
static menuitem_t LoadGame_MI[] = {menuitem_t(IT_FUNC | IT_SPACE, NULL, "", M_LoadSelect, '1'),
...
Menu LoadDef("M_LOADG", "Load Game", &MainMenuDef, ITEMS(LoadGame_MI), 80, 54, 0, &Menu::DrawLoad);
...
static menuitem_t Save_MI[] = {menuitem_t(IT_FUNC | IT_SPACE, NULL, "", M_SaveSelect, '1'),
...
Menu SaveDef("M_SAVEG", "Save Game", &MainMenuDef, ITEMS(Save_MI), 80, 54, 0, &Menu::DrawSave);
...
static consvar_t cv_menu_startmap = {
    "starting map",
    "1",
    CV_HIDDEN | CV_HANDLER,
    NULL,
...
void M_StartServer(int choice)
{
    ...
    game.SV_StartGame(
        static_cast<skill_t>(cv_menu_skill.value), cv_menu_startmap.value, 0); // launch game
    Menu::Close(true);
}
...
static menuitem_t Connect_MI[] = {
    ...
    menuitem_t(
        IT_NONE | IT_STRING | IT_WHITE, NULL, "Server Name                    ping   plys type"),
...
static consvar_t cv_menu_setupplayer = {"setupplayer",
                                        "0",
                                        CV_HIDDEN | CV_CALL,
                                        setupplayer_cons_t,
...
static consvar_t cv_menu_weaponpref = {
    "weaponpref", "1567392481457839602224446660000", CV_HIDDEN, NULL};
...
Menu SetupPlayerDef("M_MULTI",
                    "Multiplayer",
                    &OptionsDef,
                    ITEMS(SetupPlayer_MI),
                    20,
                    30,
                    0,
                    &Menu::DrawSetupPlayer,
                    M_QuitSetupPlayerMenu);
...
static void SetupPlayer_OnChange()
{
    ...
    if (current_setup >= 0)
        StorePlayerPrefs(
            LocalPlayers[current_setup]); // save the current setup before re-using the consvars
...
void Menu::DrawSetupPlayer()
{
    ...
    //  hud_font->DrawCharacter(x+98+hud_font->StringWidth(setupm_player->name.c_str()), y, '_',
    //  V_WHITEMAP | V_SCALE);
...
static menuitem_t VideoMode_MI[] = {
    menuitem_t(IT_FULL_KEYHANDLER | IT_SPACE,
               NULL,
               "",
               M_HandleVideoMode,
...
Menu VidModeDef(
    "M_VIDEO", "Video Mode", &OptionsDef, ITEMS(VideoMode_MI), 48, 36, 0, &Menu::DrawVideoMode);
...
void Menu::DrawVideoMode()
{
    ...
    {
        hud_font->DrawString(
            row, col, modedescs[i].desc, (modedescs[i].iscur ? V_WHITEMAP : 0) | V_SCALE);
    ...
    else
    {
        ...
        sprintf(temp,
                "Current default is %dx%d (%d bits)",
                cv_scr_width.value,
                cv_scr_height.value,
                cv_scr_depth.value);
...
static menuitem_t MouseOptions_MI[] = {
    ...
    menuitem_t(IT_CV_SLIDER | IT_STRING, NULL, "Mouse y-speed", &cv_mousesensy[0], 0)
    // #ifdef __MACOS__
    //  ,menuitem_t(IT_CALL | IT_WHITE, NULL, "Configure Input Sprocket..."  , macConfigureInput},
    //  60)
    // #endif
};
...
static menuitem_t Control_MI[] = {menuitem_t(IT_CONTROLSTR, NULL, "Fire", gc_fire),
...
void Menu::DrawControl()
{
    ...
    M_CenterText(y - 12,
                 (cv_menu_setupplayer.value ? "SET CONTROLS FOR SECONDARY PLAYER"
...
static menuitem_t Read1_MI[] = {menuitem_t(IT_SUBMENU | IT_SPACE, NULL, "", &ReadDef2, 0)};

Menu ReadDef1(NULL, "Readme1", &MainMenuDef, ITEMS(Read1_MI), 280, 185, 0, &Menu::DrawReadThis1);
...
static menuitem_t Read2_MI[] = {menuitem_t(IT_SUBMENU | IT_SPACE, NULL, "", &MainMenuDef, 0)};

Menu ReadDef2(NULL, "Readme2", &ReadDef1, ITEMS(Read2_MI), 330, 175, 0, &Menu::DrawReadThis2);
...
bool Menu::Responder(event_t *ev)
{
    ...
    {
        if (NowTic <= mousewait)
            return true; // just eat the event. NOTE this disables the "mouselook while menu is on"
                         // feature...
...
bool Menu::MenuResponder(int key)
{
    ...
    if (key == KEY_ESCAPE)
    {
        ...
        if (parent)
        {
            ...
            {
                if (!quitroutine())
                    return true; // we can't quit this menu (also used to set parameter from the
                                 // menu)
            }
...
void Menu::Drawer()
{
    ...
    else
    {
        ...
        // draw pause pic
        if (game.paused)
            paused_tex->Draw(game.mode < gm_heretic ? (BASEVIDWIDTH - paused_tex->worldwidth) / 2
                                                    : BASEVIDWIDTH / 2,
                             (BASEVIDHEIGHT - hud.stbarheight) / 2,
                             V_SCALE);
    }
...
void Menu::Open()
{
    ...
    for (int i = 0; i < n; i++)
    {
        Episode_MI[i].pic = game.episodes[i]->namepic.empty()
                                ? NULL
                                : game.episodes[i]->namepic.c_str(); // FIXME dangerous?
...
void Menu::Init()
{
    ...
    if (!game.inventory)
    {
        ...
        for (i = 0; i < ControlDef2.numitems; i++)
            if (Control2_MI[i].alphaKey == gc_invprev || Control2_MI[i].alphaKey == gc_invnext ||
...
{
  DrawMenu(); // use generic drawer for cursor, items and title
  hud_font->DrawString(x, y+currentMenu->items[0].alphaKey-10, "Gamma correction", V_WHITEMAP |
V_SCALE);
}
*/
