...
//-----------------------------------------------------------------------------
//
// $Id: p_enemy.cpp 531 2008-11-02 09:03:56Z smite-meister $
//
...
//
static dirtype_t opposite[] = {DI_WEST,
                               DI_SOUTHWEST,
                               DI_SOUTH,
                               DI_SOUTHEAST,
                               DI_EAST,
...
bool DActor::CheckMissileRange()
{
    ...
    if (type == MT_CYBORG || type == MT_SPIDER || type == MT_SKULL ||
...
static bool IT_FindEnemies(Thinker *th)
{
    ...
    if (!(a->flags & MF_VALIDTARGET) || !(a->flags & MF_SHOOTABLE) || (a->flags & MF_CORPSE))
    {
...
void A_Chase(DActor *actor)
{
    ...
    if (cv_fastmonsters.value && (game.mode == gm_heretic || game.mode == gm_hexen))
...
nomissile:
    ...
    if (game.multiplayer && !actor->threshold && !actor->mp->CheckSight(actor, actor->target))
    {
        if (actor->LookForEnemies(true))
            return; // got a new target
    }
...
void A_CPosRefire(DActor *actor)
{
    ...
    if (!actor->target || actor->target->health <= 0 || actor->target->flags & MF_CORPSE ||
...
void A_Tracer(DActor *actor)
{
    ...
    DActor *th = m->SpawnDActor(
        actor->pos.x - actor->vel.x, actor->pos.y - actor->vel.y, actor->pos.z, MT_SMOKE);
...
bool PIT_VileCheck(Actor *th)
{
    ...
    if (abs(thing->pos.x - viletryx) > maxdist || abs(thing->pos.y - viletryy) > maxdist)
...
{
    DActor *th =
        mo->mp->SpawnDActor(mo->pos.x + 8 * RandomS(), mo->pos.y, Random(128, 640), MT_ROCKET);
    th->vel.z = Random(0, 2);
    ...
    if (n > 0)
    {
        ...
        if (newmobj)
        {
            ...
            newmobj->reactiontime =
                ((targ->pos.y - mo->pos.y) / newmobj->vel.y).floor() / newmobj->state->tics;
        }
...
void A_PlayerScream(DActor *actor)
{
    ...
    */
    ...
    if (actor->type == MT_PLAYER_FIGHTER || actor->type == MT_PLAYER_CLERIC ||
