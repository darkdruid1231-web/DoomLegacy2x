...
//-----------------------------------------------------------------------------
//
// $Id: p_user.cpp 518 2007-12-31 02:54:58Z smite-meister $
//
...
bool PlayerPawn::UndoMorph()
{
    ...
    // spawn FX
    DActor *fog = mp->SpawnDActor(
        pos.x + 20 * Cos(yaw), pos.y + 20 * Sin(yaw), pos.z + TELEFOGHEIGHT, MT_TFOG);
    S_StartSound(fog, sfx_teleport);
...
void P_TeleportOther(Actor *v)
{
    ...
    {
        if (!(v->flags & MF_MONSTER) || v->flags2 & MF2_BOSS || v->flags2 & MF2_CANTLEAVEFLOORPIC)
            return;
...
bool P_UseArtifact(PlayerPawn *p, artitype_t arti)
{
    ...
    switch (arti)
    {
        ...
        case arti_firebomb:
            ...
            mo = p->mp->SpawnDActor(p->pos.x + 24 * finecosine[ang],
                                    p->pos.y + 24 * finesine[ang],
                                    p->pos.z - p->floorclip,
                                    MT_FIREBOMB);
            mo->owner = p;
            break;
        ...
        case arti_poisonbag:
            ...
            {
                case PCLASS_CLERIC:
                    mo = p->mp->SpawnDActor(p->pos.x + 16 * finecosine[ang],
                                            p->pos.y + 24 * finesine[ang],
                                            p->pos.z - p->floorclip + 8,
                                            MT_POISONBAG);
                    break;
                case PCLASS_MAGE:
                    mo = p->mp->SpawnDActor(p->pos.x + 16 * finecosine[ang],
                ...
                default:
                    mo = p->mp->SpawnDActor(
                        p->pos.x, p->pos.y, p->pos.z - p->floorclip + 35, MT_THROWINGBOMB);
                    if (mo)
                    {
                        mo->owner = p;
