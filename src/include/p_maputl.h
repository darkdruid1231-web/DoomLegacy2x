...
//-----------------------------------------------------------------------------
//
// $Id: p_maputl.h 522 2008-01-20 22:42:23Z smite-meister $
//
...
struct line_opening_t
{
  ...
  public:
    ...
    inline fixed_t Range()
    {
        return top - bottom;
    }
    inline fixed_t Drop()
    {
        return bottom - lowfloor;
    }
...
struct trace_t
{
  ...
  public:
    ...
    inline vec_t<fixed_t> Point(float f)
    {
        return start + delta * f;
    }

    /// Checks if trace hits a Z-plane while traversing sector s from height lastz up to fraction
    /// frac.
