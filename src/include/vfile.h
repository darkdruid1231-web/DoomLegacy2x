...
class VFile
{
  ...
  public:
    ...
    virtual bool Create(const char *fname, const char *lumpname)
    {
        return false;
    }; ///< create a single-lump wad from a file
    ...
    /// Same as above, but matches only the first four chars (in iname), and sets fullname to point
    /// to the full name.
    ...
    /// Tries to write size bytes of data item item into dest, returns the number of bytes actually
    /// written.
