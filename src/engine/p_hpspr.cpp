...
//-----------------------------------------------------------------------------
//
// $Id: p_hpspr.cpp 531 2008-11-02 09:03:56Z smite-meister $
//
...
{
    p->ammo[am_mace] -=
        cv_deathmatch.value ? wpnlev1info[wp_mace].ammopershoot : wpnlev2info[wp_mace].ammopershoot;
    ...
    if (mo)
    {
        ...
        float dummy;
        mo->target = p->AimLineAttack(
            p->yaw, AIMRANGE, dummy); // HACK to compensate SPMAngle without autoaim
    }
...
{
    p->ammo[am_skullrod] -= cv_deathmatch.value ? wpnlev1info[wp_skullrod].ammopershoot
...
void A_SkullRodStorm(DActor *actor)
{
    ...
    DActor *mo =
        actor->mp->SpawnDActor(x, y, ONCEILINGZ, mobjtype_t(MT_RAINPLR1 + actor->special2 % 4));
...
void A_FirePhoenixPL2(PlayerPawn *p, pspdef_t *psp)
{
    ...
    angle_t angle = p->yaw;
    vec_t<fixed_t> r(
        RandomS() * 2, RandomS() * 2, 26 + Tan(p->pitch) - p->floorclip); // TEST approximate
