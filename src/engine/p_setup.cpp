...
void Map::LoadThings(int lump, bool heed_spawnflags)
{
    ...
    mapthings =
        static_cast<mapthing_t *>(Z_Malloc(nummapthings * sizeof(mapthing_t), PU_LEVEL, NULL));
    ...
    for (i = 0; i < nummapthings; i++, t++)
    {
        ...
        if ((ednum >= EN_START1 && ednum <= EN_START4) ||
            (ednum >= EN_START5 && ednum <= EN_START32))
        {
            if (ednum >= EN_START5)
                ednum += 5 - EN_START5;
        ...
        else // Hexen
        {
            ...
            if (ednum == EN_HEXEN_PO_ANCHOR || ednum == EN_HEXEN_PO_SPAWN ||
                ednum == EN_HEXEN_PO_SPAWNCRUSH)
            {
        ...
        // NOTE: Spawning flags don't apply to playerstarts, teleport exits or polyobjs! Why, pray,
        // is that? wrong flags?
        if (heed_spawnflags)
...
void line_t::SetDims()
{
    ...
    slopetype = !dx ? ST_VERTICAL : !dy ? ST_HORIZONTAL : (dy / dx > 0) ? ST_POSITIVE : ST_NEGATIVE;
...
void Map::LoadLineDefs(int lump)
{
    ...
    for (int i = 0; i < numlines; i++, ld++)
    {
        ...
        else
        {
            ...
            ld->lineid = ld->tag; // NOTE: in Doom format, lineid's are equivalent to tags, in Hexen
                                  // they are different.
...
void Map::LoadSideDefs2(int lump)
{
    ...
    for (int i = 0; i < numsides; i++)
    {
        ...
        line_t *ld = sd->line;
        if (ld && ld->special ==
                      LINE_LEGACY_EXT) // TODO shouldn't all sidedefs have a corresponding linedef?
        {
            if (ld->args[0] == LINE_LEGACY_EXOTIC_TEXTURE)
                switch (ld->args[1])
                {
                    ...
                    case 2: // BOOM: 260 make middle texture translucent
                        ...
                        sd->midtexture = materials.GetMaterialOrTransmap(
                            mtex, ld->transmap); // can also be a transmap lumpname
                    ...
                    case 20:
                        if (ttex[0] == '#')
                            sd->toptexture = sd->bottomtexture =
                                0; // FIXME 3D floor translucency ((ttex[1] - '0')*100 + (ttex[2] -
                                   // '0')*10 + ttex[3] - '0') + 1;
                        else
                            sd->toptexture = sd->bottomtexture = 0;
                        sd->midtexture = materials.Get8char(mtex);
...
void Map::LoadBlockMap(int lump)
{
    ...
    if (M_CheckParm("-blockmap") || size < 6 || // smallest possible blockmap
...
void Map::LoadGLSegs(const int lump, const int glversion)
{
    ...
    for (int i = 0; i < numsegs; i++, seg++)
    {
        ...
        { // not a miniseg
            ...
            seg->frontsector = ldef->sideptr[seg->side]->sector;
            seg->backsector =
                (ldef->flags & ML_TWOSIDED) ? ldef->sideptr[seg->side ^ 1]->sector : NULL;
        }
...
bool Map::Setup(tic_t start, bool spawnthings)
{
    ...
    LoadBlockMap(lumpnum +
                 LUMP_BLOCKMAP); // loads (or builds) the blockmap, uses vertexes and lines
    ...
    // This is because the players have not been unserialized yet and hence the fclass mask cannot
    // be set.
    ...
    if (game.server)
    {
        ...
        FS_LoadScripts(
            FS_data); // load and preprocess FraggleScript scripts (needs already added players)
    }
