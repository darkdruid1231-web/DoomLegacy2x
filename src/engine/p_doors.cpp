...
//-----------------------------------------------------------------------------
//
// $Id: p_doors.cpp 518 2007-12-31 02:54:58Z smite-meister $
//
...
// constructor
vdoor_t::vdoor_t(Map *m, byte t, sector_t *s, fixed_t sp, int delay) : sectoreffect_t(m, s)
{
    type = t;
    speed = sp;
    topwait = delay;
    ...
    else
    {
        ...
        if (sec->ceilingdata) // SoM: 3/6/2000
        {
            ...
            else
            {
                ...
                CONS_Printf("Cannot spawn a vdoor_t thinker in sector %d, it already has a ceiling "
                            "action.\n",
                            sec - sectors);
                return 0;
            }
...
{
    vdoor_t *door =
        new vdoor_t(this, vdoor_t::Close | vdoor_t::Delayed, sec, VDOORSPEED, VDOORWAIT);
    door->topcount = 30 * 35;
    sec->special = 0;
}
