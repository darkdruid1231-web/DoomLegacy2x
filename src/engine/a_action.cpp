...
//-----------------------------------------------------------------------------
//
// $Id: a_action.cpp 531 2008-11-02 09:03:56Z smite-meister $
//
...
void A_PotteryCheck(DActor *actor)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (m->CheckSight(actor, pmo) &&
            Abs(R_PointToAngle2(pmo->pos.x, pmo->pos.y, actor->pos.x, actor->pos.y) - pmo->yaw) <=
                ANG45)
        {
...
void A_CorpseBloodDrip(DActor *actor)
{
    ...
    actor->mp->SpawnDActor(
        actor->pos.x, actor->pos.y, actor->pos.z + actor->height / 2, MT_CORPSEBLOODDRIP);
}
...
{
    DActor *mo =
        actor->mp->SpawnDActor(actor->pos.x, actor->pos.y, actor->pos.z + 28, MT_POISONCLOUD);
    if (!mo)
        return;
...
bool Map::EV_LocalQuake(byte *args)
{
    ...
    // Find all quake foci
    Iterate_TID iter(
        this, args[4]); // NOTE: originally had a bug, skipped the first thing in the TID array
...
void A_Quake(DActor *actor)
{
    ...
    if (actor->args[1]-- > 0)
    {
        for (i = 0; i < n; i++)
        {
            ...
            int dist = P_AproxDistance(actor->pos.x - victim->pos.x, actor->pos.y - victim->pos.y)
                           .floor() >>
                       6;
            // Tested in tile units (64 pixels)
            if (dist < actor->args[3]) // In tremor radius
            {
                // FIXME do the shaking somehow  localQuakeHappening[i] = richters; // viewx,y +=
                // -2*r..+2*r
            }
