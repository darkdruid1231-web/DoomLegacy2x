...
//-----------------------------------------------------------------------------
//
// $Id: p_fab.cpp 518 2007-12-31 02:54:58Z smite-meister $
//
...
void A_SmokeTrailer(DActor *actor)
{
    ...
    DActor *th = actor->mp->SpawnDActor(
        actor->pos.x - actor->vel.x, actor->pos.y - actor->vel.y, actor->pos.z, MT_SMOK);
...
int MT_HOLY_FX_touchfunc(DActor *d, Actor *p)
{
    ...
    if ((p->flags2 & MF2_REFLECTIVE) && (isplayer || (p->flags2 & MF2_BOSS)))
    {
...
int MT_LIGHTNING_touchfunc(DActor *d, Actor *p)
{
    ...
    if (!(temp == MT_NONE || (p->flags2 & MF2_BOSS)) || !(d->mp->maptic & 1))
    {
        if (temp == MT_CENTAUR || temp == MT_CENTAURLEADER)
