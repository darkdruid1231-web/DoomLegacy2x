...
//-----------------------------------------------------------------------------
//
// $Id: p_henemy.cpp 531 2008-11-02 09:03:56Z smite-meister $
//
...
{
    DActor *mo = actor->mp->SpawnDActor(
        actor->pos.x, actor->pos.y, actor->pos.z - mobjinfo[MT_WIZARD].height / 2, MT_WIZARD);
    if (!mo->TestLocation())
    { // Didn't fit
        mo->Remove();
        return;
...
void A_SorZap(DActor *actor)
{
    S_StartAmbSound(NULL, sfx_sorzap);
}
void A_SorRise(DActor *actor)
{
    S_StartAmbSound(NULL, sfx_sorrise);
}
void A_SorDSph(DActor *actor)
{
    S_StartAmbSound(NULL, sfx_sordsph);
}
void A_SorDExp(DActor *actor)
{
    S_StartAmbSound(NULL, sfx_sordexp);
}
void A_SorDBon(DActor *actor)
{
    S_StartAmbSound(NULL, sfx_sordbon);
}
void A_SorSightSnd(DActor *actor)
{
    S_StartAmbSound(NULL, sfx_sorsit);
}
...
void A_MinotaurDecide(DActor *actor)
{
    ...
    if (target->Top() > actor->Feet() && target->Top() < actor->Top() && dist < 8 * 64 &&
        dist > 1 * 64 && P_Random() < 150)
    { // Charge attack
    ...
    else if (target->pos.z == target->floorz && dist < 9 * 64 && P_Random() < 220)
    { // Floor fire attack
...
void A_MntrFloorFire(DActor *actor)
{
    ...
    DActor *mo = actor->mp->SpawnDActor(
        actor->pos.x + RandomS() * 4, actor->pos.y + RandomS() * 4, ONFLOORZ, MT_MNTRFX3);
    mo->target = actor->target;
...
void A_PodPain(DActor *actor)
{
    ...
    for (i = 0; i < count; i++)
    {
        goo = actor->mp->SpawnDActor(actor->pos.x, actor->pos.y, actor->pos.z + 48, MT_PODGOO);
        goo->target = actor;
        goo->vel.x = RandomS() * 2;
        goo->vel.y = RandomS() * 2;
...
void A_InitKeyGizmo(DActor *gizmo)
{
    ...
    mo = gizmo->mp->SpawnDActor(gizmo->pos.x, gizmo->pos.y, gizmo->pos.z + 60, MT_KEYGIZMOFLOAT);
    mo->SetState(state);
}
...
void A_VolcanoBlast(DActor *volcano)
{
    ...
    for (int i = 0; i < count; i++)
    {
        DActor *blast = volcano->mp->SpawnDActor(volcano->pos.x,
                                                 volcano->pos.y,
                                                 volcano->pos.z + 44,
...
{
    // FIXME TODO since we now have a pointer tracking system for Actors, maybe we should delete the
    // DActor here.
