...
int acs_t::ExecLineSpecial(int num_args)
{
    ...
    // We have extended some linedef types and hence cannot trust standard ACC to generate proper
    // opcodes for them.
...
int acs_t::Sub()
{
    int op2 = Pop(); // NOTE: for non-commutative operations we must make sure to Pop the operands
...
static acs_func_t ACS_opcode_map[] = {&acs_t::NOP, // 0
                                      &acs_t::Terminate,
                                      &acs_t::Suspend,
                                      &acs_t::PushNumber,
...
#define ACS_AUTOSTART_BASE                                                                         \
    1000 // script numbers starting from this are automatically started when map begins
...
/// Starts a new script running in the given Map, or queues it if it's intended for a currently
/// nonrunning Map. NOTE: Triggering data is not passed across Maps.
bool Map::ACS_StartScriptInMap(int mapnum, unsigned scriptnum, byte *args)
{
    ...
    {
        CONS_Printf(
            "ACS script %d was requested to be run in an unknown map %d\n", scriptnum, mapnum);
        return false; // no such map
    }

    if (m->me)
        return m->me->ACS_StartScript(
            scriptnum, args, NULL, NULL, 0); // running map, run script immediately
...
acs_t *
Map::ACS_StartScript(unsigned scriptnum, byte *args, Actor *triggerer, line_t *line, int side)
{
