...
//-----------------------------------------------------------------------------
//
// $Id: st_stuff.cpp 526 2008-01-22 23:28:24Z smite-meister $
//
...
#define NUM_PALETTES                                                                               \
    (1 + NUMREDPALS + NUMBONUSPALS + NUMPOISONPALS + 1 + NUMHOLYPALS + NUMSCOURGEPALS)
...
static const char ArtiPatchName[][10] = {{"ARTIBOX"},  // none
                                         ...
                                         {"WFR2A0"},   {"WFR3A0"}, {"WCH1A0"}, {"WCH2A0"},
                                         {"WCH3A0"},   {"WMS1A0"}, {"WMS2A0"}, {"WMS3A0"}};
...
void ST_LoadHexenData()
{
    ...
    */
    ...
    for (i = 0; i < 5; i++)
        Patch_InvBar[i + 8] = materials.GetLumpnum(startLump + i);
...
void HUD::UpdateWidgets(PlayerInfo *st_player, int vp)
{
    ...
    st_radiation =
        (st_pawn->powers[pw_ironfeet] > BLINKTHRESHOLD || st_pawn->powers[pw_ironfeet] & 8);
    ...
    if (game.mode == gm_heretic || game.mode == gm_hexen)
    {
        ...
        int left = st_player->invSlot -
                   st_curpos; // how many slots are there left of the first visible slot?
        for (i = 0; i < 7; i++)
        ...
        // book icon
        if ((st_pawn->powers[pw_weaponlevel2] > BLINKTHRESHOLD ||
             (st_pawn->powers[pw_weaponlevel2] & 16)) &&
            !st_pawn->morphTics)
            st_book = frame;
        else
            st_book = -1;
        ...
        // defense icon
        if (st_pawn->powers[pw_invulnerability] > BLINKTHRESHOLD ||
            (st_pawn->powers[pw_invulnerability] & 16))
            st_defense = frame;
        else
            st_defense = -1;
        ...
        if (rendermode == render_opengl)
        {
            float opengl_color[NUM_PALETTES][4] = {
                ...
                {0.37f,
                 1.00f,
                 0.37f,
                 1.0f}, // Doom: radiation suit (1 green), Hexen: poisoning (8 greens)
...
void HUD::CreateHexenWidgets()
{
    ...
    // inventory system
    h = new HexenHudInventory(st_x + 38,
                              st_y - 1,
                              &invopen,
                              &st_itemuse,
                              st_invslots,
    ...
    {
        /*
        // TODO fading icons: 0:nothing, <= ArmInc[cl][i]>>2:fuzz, <= ArmInc[cl][i]>>1:altfuzz,
        otherwise normal h = new HudFadeIcon(st_x+119+i*31, st_y+3, &st_armorboxes[i],
        "ARMSLOT1"+i-1); keybar.push_back(h);
        */
    }
}
...
void HUD::CreateDoomWidgets()
{
    ...
    else
    {
        ...
        for (i = 0; i < 6; i++)
        {
            ...
            h = new HudBinIcon(st_x + 111 + (i % 3) * 12,
                               st_y + 4 + (i / 3) * 10,
                               &st_weaponsowned[wsel[i]],
                               PatchArms[i][0],
                               PatchArms[i][1]);
            statusbar.push_back(h);
        }
    ...
    {
        h = new HudMultIcon(
            st_x + 239 + (i / 3) * 10, st_y + ST_KEYY[i % 3], &st_keyboxes[i + 11], PatchKeys);
        statusbar.push_back(h);
    }
}
    ...
    for (char c = *cmds++; c; c = *cmds++)
    {
        ...
        switch (c)
        {
            ...
            case 'k': // draw keys
                for (int i = 0; i < 6; i++)
                {
                    h = new HudMultIcon(st_x + 308 - (i / 3) * 10,
                                        st_y + 22 - (i % 3) * 10,
                                        &st_keyboxes[i + 11],
                                        PatchKeys);
                    overlay.push_back(h);
                ...
                {
                    h = new HudMultIcon(
                        st_x + 4 + (i / 6) * 296, 25 + (i % 6) * 30, &st_keyboxes[i], PatchKeys);
                    overlay.push_back(h);
                }
                break;
