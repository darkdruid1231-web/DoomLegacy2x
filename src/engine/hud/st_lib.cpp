...
//-----------------------------------------------------------------------------
//
// $Id: st_lib.cpp 511 2007-12-21 00:20:42Z smite-meister $
//
...
HudNumber::HudNumber(int nx, int ny, int dig, const int *number, Material **tex) : HudWidget(nx, ny)
{
    digits = dig;
    oldn = 0;
    n = number;
...
void HudNumber::Draw()
{
    ...
    if (neg)
    {
        // INumber: if (val < -9) V_DrawScaledPatch(x+1, y+1, fgbuffer, W_CachePatchName("LAME",
        // PU_CACHE));
        if (digits == 2 && lnum < -9)
            lnum = 9;
...
HudMultIcon::HudMultIcon(int nx, int ny, const int *inumber, Material **tex) : HudWidget(nx, ny)
{
    oldinum = -1;
    inum = inumber;
    icons = tex;
...
HudFace::HudFace(int nx, int ny) : HudWidget(nx, ny)
{
    priority = 0;
    count = 0;
...
HudSlider::HudSlider(int nx, int ny, const int *v, int mi, int ma, Material **t) : HudWidget(nx, ny)
{
    val = v;
    minval = mi;
    maxval = ma;
...
HudInventory::HudInventory(int nx,
                           int ny,
                           const bool *op,
                           const int *iu,
                           const inventory_t *vals,
                           const int *sel,
                           Material **nn,
                           Material **it,
                           Material **t)
