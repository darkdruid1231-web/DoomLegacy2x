...
//-----------------------------------------------------------------------------
//
// $Id: acbot.cpp 533 2009-06-11 08:31:10Z smite-meister $
//
...
static bool PTR_QuickReachable(intercept_t *in)
{
    if (in->isaline)
    {
        ...
        else
        {
            ...
            if ((r.low <= last_floorz + ((last_sector->floortype != FLOOR_WATER)
                                             ? (JumpHeight + MAXSTEP)
                                             : MAXWATERSTEP)) && // can we jump there?
...
bool ACBot::QuickReachable(Actor *g)
{
    ...
    if (last_sector == goal->subsector->sector)
        for (ffloor_t *rover = last_sector->ffloors; rover; rover = rover->next)
        {
            ...
            if (*rover->bottomheight >= pawn->Top() && goal->Feet() > *rover->bottomheight)
                return false;
        }

    return mp->blockmap->PathTraverse(
        pawn->pos, goal->pos, PT_ADDLINES | PT_ADDTHINGS, PTR_QuickReachable);
}
...
static ai_item_t item_ai[] = {
    ...
    // ammo (if dry on this type and have a weapon for it and using an inferior weapon, x, otherwise
    // if room to take it, x-3
...
#define MAXARMOR 200
    ...
    // medication, take if you need or are selfish, low health => more interest, low skill => less
    // interest
...
void ACBot::LookForThings()
{
    ...
    for (Thinker *th = mp->thinkercap.Next(); th != &mp->thinkercap; th = th->Next())
    {
        ...
        if ((actor->flags & MF_MONSTER || type == MT_BARREL) && (actor->flags & MF_SOLID))
        ...
        else if (actor->flags & MF_SPECIAL) // most likely a pickup
        {
            ...
            for (ai_item_t *t = item_ai; t->mtype != MT_NONE; t++)
                if (t->mtype == type)
                {
                    switch (t->type)
                    {
                        ...
                        case F_AMMO:
                            if (!pawn->ammo[t->misc] && HaveWeaponFor[t->misc] &&
                                bWeaponValue < 10) // fist, chainsaw
                                weight = t->weight;
                        ...
                        case F_HEAL:
                            ...
                            else if (pawn->health < t->misc)
                            {
                                weight = t->weight +
                                         (skill -
                                          sk_nightmare); // harder skill => health is valued higher
                                if (pawn->health >= 80)
                                    weight -= 3;
                                else if (pawn->health >= 60)
                                    weight -= 2;
                                else if (pawn->health >= 50)
                                    weight -= 1;
            ...
            else
            {
                ...
                node = mp->botnodes->GetNodeAt(actor->pos);
                if (node //&& P_AproxDistance(posX2x(node->x) - actor->x, posY2y(node->y) -
                         //actor->y) < (BOTNODEGRIDSIZE << 1)
                    && (weight > bUnseenItemWeight ||
                        (weight == bUnseenItemWeight && dist < bUnseenItem.dist)))
                {
                    bUnseenItem.a = actor;
                    bUnseenItem.dist = dist;
                    bUnseenItemWeight = weight;
                    // CONS_Printf("best item set to x:%d y:%d for type:%d\n", actor->pos.x.floor(),
                    // actor->pos.y.floor(), actor->type);
                }
        ...
        if (enemyFound)
        {
            ...
            {
                node = mp->botnodes->GetNodeAt(actor->pos);
                if (node && (dist < cUnseenEnemy.dist ||
                             (actor->flags & MF_PLAYER && !(cEnemy.a->flags & MF_PLAYER))))
                {
...
bool ACBot::LookForSpecialLine(fixed_t *x, fixed_t *y)
{
    for (msecnode_t *secnode = pawn->touching_sectorlist; secnode; secnode = secnode->m_tnext)
    {
        ...
        for (int i = 0; i < sec->linecount; i++)
        {
            ...
            // sector_t *specialsector = (sec == edge->frontsector) ? edge->backsector :
            // edge->frontsector;
            ...
            {
                sector_t *sector =
                    (sec == edge->frontsector) ? edge->backsector : edge->frontsector;
                ...
                for (int j = 0; j < sector->linecount; j++)
                {
                    ...
                    if (!(edge->flags & ML_REPEAT_SPECIAL) &&
                        (spac == SPAC_USE || spac == SPAC_PASSUSE))
                    //&& (special == 11 || special == 21 || special == 200))
                    {
                        *x = (edge->v1->x + edge->v2->x) / 2;
                        *y = (edge->v1->y + edge->v2->y) / 2;
...
static ai_weapon_t ai_weapon_data[NUMWEAPONS] = {
    ...
    */
    {MELEE, 0.1, MT_NONE, 0},
    {MELEE, 1, MT_NONE, 0},
    {INSTANT | AF, 0.5, MT_NONE, 0},
    {INSTANT, 30, MT_NONE, 0},
    {INSTANT, 55, MT_NONE, 0},
    {INSTANT | AF, 50, MT_NONE, 0},
    {MISSILE, 50, MT_ROCKET, 100},
    {MISSILE | AF, 50, MT_PLASMA, 0},
    {MISSILE, 20, MT_BFG, 0},
    ...
    */
    ...
    {MELEE, 1, MT_NONE, 0},
    {INSTANT | AF, 0.5, MT_NONE, 0},
    {MISSILE, 35, MT_CRBOWFX1, 0},
    {INSTANT | AF, 50, MT_NONE, 0},
    {MISSILE, 50, MT_PHOENIXFX1, 100},
    {MISSILE | AF, 55, MT_HORNRODFX1, 0},
    {MISSILE | AF, 50, MT_MACEFX1, 0},
    {MELEE, 0.1, MT_NONE, 0},
    ...
    */
    {MELEE, 10, MT_NONE, 0},
    {MELEE, 1, MT_NONE, 0},
    {INSTANT | AF, 20, MT_NONE, 0},
    {MELEE, 30, MT_NONE, 0},
    {MISSILE, 30, MT_CSTAFF_MISSILE, 0},
    {MISSILE, 30, MT_SHARDFX1, 0},
    {MISSILE, 40, MT_HAMMER_MISSILE, 0},
    {INSTANT, 40, MT_NONE, 0},
    {MISSILE, 40, MT_LIGHTNING_FLOOR, 0},
    {MISSILE, 50, MT_FSWORD_MISSILE, 0},
    {MISSILE, 50, MT_HOLY_MISSILE, 0},
    {MISSILE, 50, MT_MSTAFF_FX2, 0},
    {MELEE, 0.1, MT_NONE, 0}};
...
void ACBot::ChangeWeapon()
{
    ...
    for (i = 0; i < NUMWEAPONS; i++)
    {
        ...
        weapon_usable[i] =
            pawn->weaponowned[i] &&
            (at == am_noammo ||
             ((at == am_manaboth) ? min(pawn->ammo[am_mana1], pawn->ammo[am_mana2])
                                  : pawn->ammo[at]) >= pawn->weaponinfo[i].ammopershoot);
...
void ACBot::AimWeapon()
{
    ...
    if (dist > wdata->dangerdist)
    {
        ...
        cmd->pitch =
            int(atan(((temp.z - pawn->pos.z + (dest->height - pawn->height) / 2) / dist).Float()) *
                (ANG180 / M_PI)) >>
            16;

        int spread = RandomS() * ANG45; // was P_SignedRandom()<<21;

        if (P_AproxDistance(dest->vel.x, dest->vel.y) >
            8) // enemy is moving reasonably fast, so not perfectly acurate
        {
        ...
        if (unsigned(abs(delta)) < ANG45)
        {
            ...
            {
                if (skill <=
                    sk_medium) // check skill, if anything but nightmare bot aim is imperfect
...
void ACBot::BuildInput(PlayerInfo *p, int elapsed)
{
    ...
    else
    {
        ...
        {
            // If a nearby item has a good weight, get it no matter what. Else only if we have no
            // target/enemy.
        ...
        else if (cEnemy.a)
        {
            ...
            if (dist <= ai_strafedist[skill] ||
                (pp && skill >= sk_medium &&
                 (ai_weapon_data[pp->readyweapon].type & TYPEMASK) == INSTANT))
        ...
        else
        {
            ...
            else if (fTeammate.a)
            {
                ...
                dist = P_AproxDistance(pawn->pos.x - fTeammate.a->pos.x,
                                       pawn->pos.y - fTeammate.a->pos.y)
                           .floor();
                if (dist > 100)
                {
            ...
            else if (lastTarget && (lastTarget->flags & MF_SOLID))
            {
                ...
                if ((pawn->vel.x == 0 && pawn->vel.y == 0) ||
                    !mp->botnodes->DirectlyReachable(
                        NULL, pawn->pos.x, pawn->pos.y, lastTargetX, lastTargetY))
            ...
            else
            {
                ...
                if (bUnseenItem.a)
                {
                    ...
                    // CONS_Printf("found a best item at x:%d, y:%d\n", bUnseenItem->x>>FRACBITS,
                    // bUnseenItem.a->y>>FRACBITS);
                }
                ...
                if (destination)
                {
                    ...
                    if (!path.empty() &&
                        P_AproxDistance(pawn->pos.x - path.front()->mx,
                                        pawn->pos.y - path.front()->my) < (BotNodes::GRIDSIZE << 1))
                    {
                    ...
                    if (path.empty() ||
                        !mp->botnodes->DirectlyReachable(
                            NULL, pawn->pos.x, pawn->pos.y, path.front()->mx, path.front()->my))
                        if (!mp->botnodes->FindPath(
                                path, mp->botnodes->GetNodeAt(pawn->pos), destination))
                        {
                            // CONS_Printf("Bot stuck at x:%d y:%d could not find a path to x:%d
                            // y:%d\n",pawn->x>>FRACBITS, pawn->y>>FRACBITS,
                            // posX2x(destination->x)>>FRACBITS, posY2y(destination->y)>>FRACBITS);
                    ...
                    if (!path.empty())
                    {
                        ...
                        // CONS_Printf("it has a distance %d\n", (P_AproxDistance(pawn->x -
                        // nextItemNode->x, pawn->y - nextItemNode->y)>>FRACBITS));
        ...
        fixed_t cpx =
            (forwardmove * finecosine[forwardAngle] + sidemove * finecosine[sideAngle]) >> 5;
        ...
        if (blocked)
        {
            if (++blockedcount > 20 && (P_AproxDistance(pawn->vel.x, pawn->vel.y) < 4 ||
                                        (ccc->block_thing && (ccc->block_thing->flags & MF_SOLID))))
                avoidtimer = 20;

            if (ccc->op.bottom - pawn->Feet() > MAXSTEP &&
                (ccc->op.bottom - pawn->Feet() <= MAXWATERSTEP ||
                 (ccc->op.bottom - pawn->Feet() <= 45 &&
                  pawn->subsector->sector->floortype != FLOOR_WATER))) // FIXME cv_jumpspeed
                cmd->buttons |= ticcmd_t::BT_JUMP;

            for (unsigned i = 0; i < ccc->spechit.size(); i++)
                if (ccc->spechit[i]->backsector)
                {
                    if (!ccc->spechit[i]->backsector->ceilingdata &&
                        !ccc->spechit[i]->backsector->floordata)
                        cmd->buttons |= ticcmd_t::BT_USE;
                }
        }
        else
