...
//-----------------------------------------------------------------------------
//
// $Id: b_path.cpp 533 2009-06-11 08:31:10Z smite-meister $
//
...
SearchNode_t::SearchNode_t(int nx, int ny, fixed_t x, fixed_t y)
{
    ...
    //  newnode->mo = P_SpawnMobj(posX2x(newnode->x), posY2y(newnode->y),
    //  GetSubsector(posX2x(newnode->x), posY2y(newnode->y))->sector->floorheight, MT_MISC49);
}
    ...
    if (in->isaline)
    {
        ...
        switch (line->special)
        {
            ...
            default:
            {
                ...
                // Determine if looking at backsector/frontsector.
                sector_t *s =
                    (line->backsector == last_sector) ? line->frontsector : line->backsector;
                ...
                if (s != oksector)
                {
                    ...
                    if (floorheight > last_sector->floorheight + 45 ||
                        (floorheight > last_sector->floorheight + 37 &&
                         last_sector->floortype == FLOOR_WATER))
...
bool BotNodes::DirectlyReachable(Actor *mo, fixed_t x, fixed_t y, fixed_t destx, fixed_t desty)
{
    ...
    if ((nx >= 0) && (nx < xSize) && (ny >= 0) && (ny < ySize))
    {
        ...
        if (mo)
        {
            ...
            if (mp->blockmap->IterateLinesRadius(
                    destx, desty, mo->radius, PIT_BBoxFit) // does it fit there?
                /*
                && mp->PathTraverse(x, y, destx - 1, desty + 1, PT_ADDLINES|PT_ADDTHINGS,
                PTR_BotPath)
                && mp->PathTraverse(x, y, destx + 1, desty + 1, PT_ADDLINES|PT_ADDTHINGS,
                PTR_BotPath)
                && mp->PathTraverse(x, y, destx - 1, desty - 1, PT_ADDLINES|PT_ADDTHINGS,
                ...
                && mp->PathTraverse(x, y, destx + 1, desty - 1, PT_ADDLINES|PT_ADDTHINGS,
                PTR_BotPath)
                && mp->PathTraverse(x - 1, y + 1, destx, desty, PT_ADDLINES|PT_ADDTHINGS,
                PTR_BotPath)
                && mp->PathTraverse(x + 1, y + 1, destx, desty, PT_ADDLINES|PT_ADDTHINGS,
                PTR_BotPath)
                && mp->PathTraverse(x - 1, y - 1, destx, desty, PT_ADDLINES|PT_ADDTHINGS,
...
bool BotNodes::FindPath(list<SearchNode_t *> &path, SearchNode_t *start, SearchNode_t *dest)
{
    ...
    if (start) // || P_AproxDistance(pawn->x - start->x, pawn->y - start->y) > (BOTNODEGRIDSIZE<<1))
               // //no nodes can get here
    {
        path.clear();
        ...
        start->f = start->heuristic =
            P_AproxDistance(start->gx - dest->gx, start->gy - dest->gy) * 10000;
                ...
                if (botteledestfound)
                {
                    ...
                    if (!botNodeArray[nx][ny])
                    {
                        temp->dir[BDI_TELEPORT] = botNodeArray[nx][ny] =
                            new SearchNode_t(nx, ny, posX2x(nx), posY2y(ny));
