...
//-----------------------------------------------------------------------------
//
// $Id: p_xenemy.cpp 570 2009-11-27 23:57:47Z smite-meister $
//
...
// Returns 1 if source should turn in the positive direction, 0 if in the negative direction, by the
// amount 0 <= delta <= ANG180.
...
static bool IT_XMinotaur(Actor *mo)
{
    ...
    if (!(mo->flags & MF_MONSTER) || mo->health <= 0 || !(mo->flags & MF_SHOOTABLE))
        return true;
    ...
    if (dist > MINOTAUR_LOOK_DIST || mo == mino->owner || mo == mino || mo->owner == mino->owner)
        return true;
...
bool PIT_XMinotaurServantLook(Actor *mo)
{
    if ((mo->flags & MF_VALIDTARGET) // meaning "monster or player"
        && (mo->flags & MF_SHOOTABLE) && !(mo->flags2 & MF2_DORMANT) &&
...
void A_MinotaurLook(DActor *actor)
{
    ...
    {
        mino = actor; // special case for minotaur blocksearch since the search may be centered on
                      // master
...
void A_XMinotaurDecide(DActor *actor)
{
    ...
    if (target->Top() > actor->Feet() && target->Top() < actor->Top() && dist < 16 * 64 &&
        dist > 1 * 64 && P_Random() < 230)
    { // Charge attack
    ...
    else if (target->pos.z == target->floorz && dist < 9 * 64 && P_Random() < 100)
    { // Floor fire attack
...
void A_MntrFloorFire(DActor *actor)
{
  ...
  mo = actor->mp->SpawnDActor(actor->pos.x + RandomS()*4, actor->pos.y + RandomS()*4, ONFLOORZ,
MT_MNTRFX3); mo->target = actor->target; mo->vel.x = 1; // Force block checking
...
void A_CentaurDropStuff(DActor *actor)
{
    ...
    mo = actor->mp->SpawnDActor(actor->pos.x, actor->pos.y, actor->pos.z + 45, MT_CENTAUR_SHIELD);
    if (mo)
    {
    ...
    mo = actor->mp->SpawnDActor(actor->pos.x, actor->pos.y, actor->pos.z + 45, MT_CENTAUR_SWORD);
    if (mo)
    {
        angle = actor->yaw - ANG90;
        mo->vel.z = Random(8, 12);
...
void A_BishopPuff(DActor *actor)
{
    ...
    mo = actor->mp->SpawnDActor(actor->pos.x, actor->pos.y, actor->pos.z + 40, MT_BISHOP_PUFF);
    if (mo)
    {
        mo->vel.z = 0.5f;
    }
...
static void DragonSeek(DActor *actor, angle_t thresh, angle_t turnMax)
{
    ...
    { // Hit the target thing
        if (actor->target && P_Random() < 200)
        {
            ...
            for (i = 0; i < 5; i++)
            {
                if (!t->args[i] || !(node[i] = Map::Iterate_TID(actor->mp, t->args[i]).Next()))
                    continue;
...
void A_DragonFX2(DActor *actor)
{
    ...
    for (int i = 1 + (P_Random() & 3); i; i--)
    {
        DActor *mo = actor->mp->SpawnDActor(actor->pos.x + Random(-32, 32),
                                            actor->pos.y + Random(-32, 32),
                                            actor->pos.z + Random(-8, 8),
                                            MT_DRAGON_FX2);
...
void A_DropMace(DActor *actor)
{
    ...
    mo = actor->mp->SpawnDActor(actor->pos.x, actor->pos.y, actor->Center(), MT_ETTIN_MACE);
    if (mo)
    {
        mo->vel.x = Random(-4, 4);
        mo->vel.y = Random(-4, 4);
...
static DActor *
P_SpawnMissileXYZ(fixed_t x, fixed_t y, fixed_t z, Actor *source, Actor *dest, mobjtype_t type)
{
    angle_t an;
    int dist;
...
void A_SorcBallOrbit(DActor *actor)
{
    ...
    switch (mode)
    {
        ...
        case SORC_STOPPING: // Balls stopping
            if (parent->special2 == actor->type && parent->args[1] > SORCBALL_SPEED_ROTATIONS &&
...
void A_FreezeDeathChunks(DActor *actor)
{
    ...
    for (i = 12 + (P_Random() & 15); i >= 0; i--)
    {
        mo = actor->mp->SpawnDActor(actor->pos.x + Random(-1, 1) * actor->radius,
                                    ...
                                    actor->pos.z + Random() * actor->height,
                                    MT_ICECHUNK);
    ...
    if(actor->player)
      { // attach the player's view to a chunk of ice
        mo = actor->mp->SpawnDActor(actor->pos.x, actor->pos.y, actor->pos.z+VIEWHEIGHT,
    MT_ICECHUNK); mo->SetState(S_ICECHUNK_HEAD); mo->vel.z = FixedDiv(mo->pos.z-actor->pos.z,
    actor->height)<<2; mo->vel.x = RandomS()*2; mo->vel.y = RandomS()*2; mo->flags2 |=
    MF2_ICEDAMAGE; // used to force blue palette mo->flags2 &= ~MF2_FLOORCLIP; mo->player =
    actor->player; actor->player = NULL; mo->health = actor->health; mo->yaw = actor->yaw;
        mo->player->mo = mo;
        mo->player->lookdir = 0;
      }
    */
...
DActor *
P_SpawnKoraxMissile(fixed_t x, fixed_t y, fixed_t z, Actor *source, Actor *dest, mobjtype_t type)
{
    angle_t an;
    int dist;
...
void A_KSpiritRoam(DActor *actor)
{
    ...
    else
    {
        if (actor->owner)
        {
            A_KSpiritSeeker(actor, actor->args[0] * ANGLE_1, actor->args[0] * ANGLE_1 * 2);
        }
