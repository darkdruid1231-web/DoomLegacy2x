...
bool Map::EV_ActivatePolyobj(
    unsigned id, int type, float speed, angle_t angle, float dist, int delay, bool override)
{
...
{
    // find the unique vertices of the polyobj (with LINE_PO_EXPLICIT you can have subcycles and
    // hence shared vertices!)
    set<vertex_t *> points;
    int n_lines = lines.size();
    // if (explicit)
    for (int k = 0; k < n_lines; k++)
        if (!lines[k])
        {
            bad = true;
            CONS_Printf(
                "Error: Polyobj %d: Missing LINE_PO_EXPLICIT order number %d!\n", id, k + 1);
            return false;
        }
        else
        {
            points.insert(lines[k]->v1);
            points.insert(lines[k]->v2); // TODO with LINE_PO_EXPLICIT, the polyobj can in principle
                                         // consist of disjoint lines... bad?
        }
    ...
    current_points =
        static_cast<vertex_t **>(Z_Malloc(num_points * sizeof(vertex_t *), PU_LEVEL, 0));
...
void blockmap_t::PO_Link(polyobj_t *p)
{
    ...
    for (int j = p->bbox[BOXBOTTOM] * width; j <= p->bbox[BOXTOP] * width; j += width)
    {
        for (int i = p->bbox[BOXLEFT]; i <= p->bbox[BOXRIGHT]; i++)
        {
            ...
            {
                // allocate a new link
                cells[j + i].polys = link =
                    static_cast<polyblock_t *>(Z_Malloc(sizeof(polyblock_t), PU_LEVEL, 0));
                link->next = NULL;
                link->prev = NULL;
                link->polyobj = p;
                continue;
...
bool Map::PO_FindLines(polyobj_t *p, line_t *line)
{
    ...
    for (int k = 0; k < numlines; k++)
        if (*lines[k].v1 == end)
        {
            if (*lines[k].v2 == end)
            {
                CONS_Printf("Error: Polyobj %d: Linedef %d in a LINE_PO_START cycle starting from "
                            "linedef %d has identical endpoints.\n",
                            p->id,
                            k,
                            line - lines);
                return false;
            ...
            {
                CONS_Printf("Error: Polyobj %d: Cycle starting from LINE_PO_START linedef %d has "
                            "more than %d lines (or a subcycle).\n",
                            p->id,
                            line - lines,
                            PO_MAXLINES);
                return false;
...
void Map::PO_Init()
{
    ...
    for (int i = 0; i < NumPolyobjs; i++)
    {
        ...
        unsigned id =
            mt->angle; // For a EN_PO_SPAWN* thing, the "angle" field contains the PO number.
        ...
        if (id == 0)
            CONS_Printf(
                "Warning: Polyobj with polyNumber zero found! Strange things may happen.\n");
    ...
    // Search linedefs for the polyobj definitions. Single-pass algorithm, with the exception of
    // finding the LINE_PO_START cycles.
    for (int i = 0; i < numlines; i++)
    {
        ...
        if (line->special == LINE_PO_START)
        {
            ...
            if (!p)
            {
                CONS_Printf("Error: Polyobj %d (Initialized from LINE_PO_START linedef %d): No "
                            "anchor point found!\n",
                            id,
                            i);
                continue;
            ...
            {
                CONS_Printf(
                    "Error: Polyobj %d: Multiple initializations! (LINE_PO_START linedef %d)\n",
                    id,
                    i);
                continue;
            }
        ...
        else if (line->special == LINE_PO_EXPLICIT)
        {
            ...
            if (!p)
            {
                CONS_Printf("Error: Polyobj %d (Initialized from LINE_PO_EXPLICIT linedef %d): No "
                            "anchor point found!\n",
                            id,
                            i);
                continue;
            ...
            unsigned n = line->args[1]; // alkaa 1:st�!!

            if (n == 0)
            {
                CONS_Printf("Error: Polyobj %d: LINE_PO_EXPLICIT linedef %d has zero as its order "
                            "number!\n",
                            id,
                            i);
                continue;
            ...
            {
                p->bad = true;
                CONS_Printf("Error: Polyobj %d: LINE_PO_EXPLICIT linedef %d has an order number "
                            "greater than %d.\n",
                            id,
                            i,
                            PO_MAXLINES);
            ...
            if (p->lines[n])
            {
                CONS_Printf("Error: Polyobj %d: The order number of LINE_PO_EXPLICIT linedef %d "
                            "has already been taken by line %d!\n",
                            id,
                            i,
                            p->lines[n] - lines);
                continue;
