...
//-----------------------------------------------------------------------------
//
// $Id: g_actor.cpp 526 2008-01-22 23:28:24Z smite-meister $
//
...
/*!
  ...
  Has typically the flags MF_SOLID | MF_SHOOTABLE | MF_COUNTKILL and the flags2 MF2_PUSHWALL |
  MF2_MCROSS.
*/
...
/*!
  ...
  Has the flags MF_FLOAT | MF_DROPOFF | MF_NOGRAVITY in addition to the normal @ref actor_monster
  flags. In DActor::P_Move, if it encounters a wall with an opening during its move, it will try to
  float vertically towards the opening and set the flag MFE_INFLOAT. If it has a target, during
  ZMovement it will float towards it in the vertical direction unless MFE_INFLOAT is on.
*/
...
/*!
  ...
  MF2_SEEKERMISSILE homes towards its target. If reflected from a MF2_REFLECTIVE target, owner
  becomes target and v.v.

  When a missile Touch:es an Actor and explodes, the Actor is stored in its target field.
  Since missiles use different action functions than monsters, they also use their sounds a little
  differently. seesound: launch, painsound: floor/wall bounce, deathsound: explosion/hit,
...
/*!
  ...
  \ingroup g_actor
  Item that can be picked up by the players, e.g. a health bonus. Definition: has the flag
  MF_SPECIAL. Often also has the flags MF_COUNTITEM and MF2_FLOATBOB.
*/

/*!
  \page actor_floatbob Floatbob
  \ingroup g_actor
  MF2_FLOATBOB makes the item smoothly bob up and down: pos.z = floorz + special1 + 8*sin(2pi *
  reactiontime / 64), where reactiontime is incremented each tic.
*/
...
///  Normal constructor
Actor::Actor(fixed_t nx, fixed_t ny, fixed_t nz) : Thinker(), pos(nx, ny, nz)
{
...
void Actor::XYMovement()
{
    ...
    fixed_t xmove =
        vel.x; // this is so that we can change velocity in the collision code during the move
    fixed_t ymove = vel.y;
            ...
            else if (flags & MF_MISSILE)
            {
                ...
                if (flags2 & MF2_FULLBOUNCE)
                {
                    ...
                    if (t && t->info->painsound)
                        S_StartSound(t, t->info->painsound); // for missiles, this is the wall/floor
                                                             // bounce sound
                ...
                if (ccc->skyimpact)
                {
                    ...
                    // if (type == MT_HOLY_FX) ExplodeMissile(); // TODO some things do explode
                    // against sky
                    Remove();
                    return;
                }
                ...
                if (ccc->block_line && !(flags & MF_NOSCORCH))
                {
                    ...
                    ccc->block_line->AddWallSplat("A_DMG3",
                                                  P_PointOnLineSide(pos.x, pos.y, ccc->block_line),
                                                  pos.z,
                                                  frac,
                                                  SPLATDRAWMODE_SHADE);
                }
...
void Actor::LandOnThing(Actor *onmobj)
{
    ...
    if (cv_fallingdamage.value)
    {
        ...
        if (dv > 8 && dv > cv_fallingdamage.value)
            FallingDamage(dv);
...
void Actor::ZMovement()
{
    ...
    if ((flags & MF_FLOAT) && target && !(eflags & (MFE_SKULLFLY | MFE_INFLOAT)))
    {
        ...
        fixed_t delta = target->pos.z + (height >> 1) -
                        pos.z; // assumed to be taller than target so this looks good...
        ...
        if (thing)
        {
                     ...
                     thing->Top() <= Feet() + MAXSTEP)
            {
                ...
                if (pp && pp->player)
                {
                    ...
                    pp->player->deltaviewheight =
                        (cv_viewheight.value - pp->player->viewheight) >> 3;
                }
                pos.z = thing->Top();
                vel.z = thing->vel.z;
    ...
    */
    ...
    if (game.mode == gm_hexen)
    {
        ...
        switch (floortype)
        {
            ...
            case FLOOR_LAVA:
                ...
                {
                    p = mp->SpawnSplash(
                        pos, floorz, SFX_LAVA_SIZZLE, MT_XLAVASPLASH, MT_NONE, false);
                    if (p)
                        p->floorclip += SMALLSPLASHCLIP;
                }
                else
                {
                    p = mp->SpawnSplash(
                        pos, floorz, SFX_LAVA_SIZZLE, MT_XLAVASPLASH, MT_XLAVASMOKE, false);
                    if (p)
            ...
            case FLOOR_SLUDGE:
                ...
                {
                    p = mp->SpawnSplash(
                        pos, floorz, SFX_SLUDGE_GLOOP, MT_XSLUDGESPLASH, MT_NONE, false);
                    if (p)
                        p->floorclip += SMALLSPLASHCLIP;
                }
                else
                {
                    p = mp->SpawnSplash(
                        pos, floorz, SFX_SLUDGE_GLOOP, MT_XSLUDGESPLASH, MT_XSLUDGECHUNK, false);
                    if (p)
    ...
    else if (game.mode == gm_heretic)
    {
        switch (floortype)
        {
            ...
            case FLOOR_SLUDGE:
                p = mp->SpawnSplash(
                    pos, floorz, sfx_splash, MT_SLUDGESPLASH, MT_SLUDGECHUNK, false);
                if (p)
                    p->vel.Set(RandomS(), RandomS(), 1 + Random());
                break;
        }
...
// normal constructor
DActor::DActor(fixed_t nx, fixed_t ny, fixed_t nz, const ActorInfo *ai) : Actor(nx, ny, nz)
{
    info = ai;
    type = ai->GetMobjType();
...
void DActor::Think()
{
    ...
    if ((oldeflags & MFE_SKULLFLY) && !(eflags & MFE_SKULLFLY))
        SetState(game.mode >= gm_heretic
                     ? info->seestate
                     : info->spawnstate); // TODO depends on monster, not gametype
...
DActor *DActor::SpawnMissile(Actor *dest, mobjtype_t type, fixed_t h)
{
    ...
    int dist =
        int(P_AproxDistance(dest->pos.x - pos.x, dest->pos.y - pos.y).Float() / th->info->speed);
