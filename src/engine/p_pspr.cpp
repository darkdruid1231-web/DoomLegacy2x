...
//-----------------------------------------------------------------------------
//
// $Id: p_pspr.cpp 531 2008-11-02 09:03:56Z smite-meister $
//
...
void PlayerPawn::FireWeapon()
{
    ...
    {
        newstate = refire ? weaponinfo[readyweapon].holdatkstate : weaponinfo[readyweapon].atkstate;
...
void A_WeaponReady(PlayerPawn *p, pspdef_t *psp)
{
    ...
    if (p->readyweapon == wp_chainsaw && psp->state == &weaponstates[S_SAW])
    {
        S_StartAttackSound(p, sfx_sawidl);
    }
    // Check for staff PL2 active sound
    if ((p->readyweapon == wp_staff) && (psp->state == &weaponstates[S_STAFFREADY2_1]) &&
        P_Random() < 128)
    {
...
void A_ReFire(PlayerPawn *p, pspdef_t *psp)
{
    ...
    if ((p->player->cmd.buttons & ticcmd_t::BT_ATTACK) && p->pendingweapon == wp_none && p->health)
    {
        p->refire++;
        p->FireWeapon();
    }
...
{
    p->ammo[p->weaponinfo[p->readyweapon].ammo] -= p->weaponinfo[p->readyweapon].ammopershoot;
    p->SetPsprite(ps_flash,
                  weaponstatenum_t(p->weaponinfo[p->readyweapon].flashstate + (P_Random() & 1)));
...
void A_FireCGun(PlayerPawn *p, pspdef_t *psp)
{
    ...
    p->SetPsprite(ps_flash,
                  weaponstatenum_t(p->weaponinfo[p->readyweapon].flashstate + psp->state -
                                   &weaponstates[S_CHAIN1]));
    bulletsine = P_BulletSlope(p);
    P_GunShot(p, !p->refire);
}
...
void A_BFGSpray(DActor *mo)
{
    ...
    for (int i = 0; i < 40; i++)
    {
        ...
        DActor *extrabfg = mo->mp->SpawnDActor(
            targ->pos.x, targ->pos.y, targ->pos.z + (targ->height >> 2), MT_EXTRABFG);
        ...
        // to get correct recoil, we must use either the player or the BFG ball as the inflictor
        // (matter of taste)
