...
//-----------------------------------------------------------------------------
//
// $Id: p_floor.cpp 485 2007-08-05 13:52:17Z smite-meister $
//
...
int Map::T_MovePlane(sector_t *sector, fixed_t speed, fixed_t dest, int crush, int floorOrCeiling)
{
    ...
    switch (floorOrCeiling)
    {
        case 0: // Moving a floor
            ...
            {
                // floor going down
                if (boomsupport && sector->heightsec != -1 &&
                    sector->heightsec_type == sector_t::CS_water)
                {
            ...
            else
            {
                ...
                destheight =
                    (!boomsupport || dest < sector->ceilingheight) ? dest : sector->ceilingheight;
        ...
        case 1: // moving a ceiling
            ...
            if (speed < 0)
            {
                ...
                destheight =
                    (!boomsupport || dest > sector->floorheight) ? dest : sector->floorheight;
...
// TODO this could be used to do the Boom stairs as well (and to remove stairlocks etc. from
// sector_t)
void stair_t::Think()
{
    if (resetcount)
    ...
    switch (state)
    {
        ...
        case Moving:
            ...
            if (stepdelay)
                if ((speed >= 0 && sector->floorheight >= delayheight) ||
                    (speed < 0 && sector->floorheight <= delayheight))
                {
                    state = Waiting;
                    wait = stepdelay;
...
int Map::EV_BuildHexenStairs(
    unsigned tag, int type, fixed_t speed, fixed_t stepdelta, int resetdelay, int stepdelay)
{
    ...
    while ((secnum = FindSectorFromTag(tag, secnum)) >= 0)
    {
        ...
        // TODO wrong, if there are several tagged sectors with different heights/textures... was
        // like this in Hexen.
    ...
    while ((secnum = FindSectorFromTag(tag, secnum)) >= 0)
    {
        ...
        for (i = 0; i < s2->linecount; i++)
        {
            ...
            floor_t *floor = new floor_t(
                this, floor_t::AbsHeight | floor_t::SetTxTy, s2, sspeed, 0, s3->floorheight);
...
// constructor
elevator_t::elevator_t(
    Map *m, int ty, sector_t *sec, fixed_t sp, fixed_t height_f, fixed_t height_c, int cr)
    : sectoreffect_t(m, sec)
{
    type = ty;
    crush = cr;
    ...
    switch (ty)
    {
        ...
        case ClosePillar:
            ...
            if (floordest - sec->floorheight >= sec->ceilingheight - ceilingdest)
            {
                floorspeed = sp;
                ceilingspeed =
                    -(sec->ceilingheight - ceilingdest) * (sp / (floordest - sec->floorheight));
            }
            else
            {
                floorspeed =
                    (floordest - sec->floorheight) * (sp / (sec->ceilingheight - ceilingdest));
                ceilingspeed = -sp;
            }
            mp->SN_StartSequence(&sec->soundorg, SEQ_PLAT + sec->seqType);
            return;
...
int Map::EV_DoElevator(
    unsigned tag, int type, fixed_t speed, fixed_t height_f, fixed_t height_c, int crush)
{
    int secnum = -1;
    int rtn = 0;
