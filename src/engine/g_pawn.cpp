...
int ArmorIncrement[NUMCLASSES][NUMARMOR] = {
    {0, 0, 0, 0, 0}, {0, 25, 20, 15, 5}, {0, 10, 25, 5, 20}, {0, 5, 15, 10, 25}, {0, 0, 0, 0, 0}};

int MaxArmor[NUMCLASSES] = {200, 100, 90, 80, 5};

vector<PawnAI *> pawn_aid;

PawnAI::PawnAI(const ActorInfo &base, pclass_t cls, weapontype_t bw) : ActorInfo(base)
{
    pclass = cls;
    bweapon = bw;
    bammo = 50;
...
} pawndata[] = {{MT_PLAYER, PCLASS_NONE, wp_pistol, 50, MT_NONE}, // 0
...
PlayerPawn::PlayerPawn() : Pawn()
{
    ...
    // NOTE! This constructor is used when Unserializing or Ghosting, so not everything is
    // initialized!
    player = NULL;
    weaponinfo = NULL;
...
Pawn::Pawn(fixed_t x, fixed_t y, fixed_t z, int type) : Actor(x, y, z)
{
    // TEMP
    if (type < 0)
    {
...
PlayerPawn::PlayerPawn(fixed_t nx, fixed_t ny, fixed_t nz, int type) : Pawn(nx, ny, nz, type)
{
...
void PlayerPawn::Think()
{
    ...
    */
    ...
    if (powers[pw_invulnerability])
    {
        if (powers[pw_invulnerability] > BLINKTHRESHOLD || (powers[pw_invulnerability] & 8))
    ...
    {
        if (powers[pw_infrared] > BLINKTHRESHOLD || (powers[pw_infrared] & 8))
        {
            // almost full bright
            fixedcolormap = 1;
        }
...
void PlayerPawn::DeathThink()
{
    ...
    if (attacker != NULL && attacker != this)
    {
        ...
        // if (dist)
        //  pitch = FixedMul ((160<<FRACBITS), FixedDiv (attacker->z + (attacker->height>>1), dist))
        //  >>FRACBITS;
        // else pitch = 0;
...
#define F_NORM 0.90625f
    ...
    bool onground =
        (pos.z <= floorz) || (eflags & (MFE_ONMOBJ | MFE_FLY)) || (cheats & CF_FLYAROUND);
    ...
    if (cmd->buttons & ticcmd_t::BT_JUMP)
    {
        if (eflags & MFE_FLY)
            ...
            vel.z = 0.5f;
                 ...
                 cv_jumpspeed.value) // can't jump while in air or while jumping
        {
...
void PlayerPawn::XYFriction(fixed_t oldx, fixed_t oldy)
{
    ...
    if (player && vel.x > -STOPSPEED && vel.x < STOPSPEED && vel.y > -STOPSPEED &&
        vel.y < STOPSPEED && (player->cmd.forward == 0 && player->cmd.side == 0))
    {
...
bool PlayerPawn::CanUnlockGenDoor(line_t *line)
{
    ...
    {
        case AnyKey_:
            if (!(keycards & it_redcard) && !(keycards & it_redskull) &&
                !(keycards & it_bluecard) && !(keycards & it_blueskull) &&
                !(keycards & it_yellowcard) && !(keycards & it_yellowskull))
            {
            ...
            break;
        case RCard:
            if (!(keycards & it_redcard) && (!skulliscard || !(keycards & it_redskull)))
            {
            ...
            break;
        case BCard:
            if (!(keycards & it_bluecard) && (!skulliscard || !(keycards & it_blueskull)))
            {
            ...
            break;
        case YCard:
            if (!(keycards & it_yellowcard) && (!skulliscard || !(keycards & it_yellowskull)))
            {
            ...
            break;
        case RSkull:
            if (!(keycards & it_redskull) && (!skulliscard || !(keycards & it_redcard)))
            {
            ...
            break;
        case BSkull:
            if (!(keycards & it_blueskull) && (!skulliscard || !(keycards & it_bluecard)))
            {
            ...
            break;
        case YSkull:
            if (!(keycards & it_yellowskull) && (!skulliscard || !(keycards & it_yellowcard)))
            {
            ...
            break;
        case AllKeys:
            if (!skulliscard && (!(keycards & it_redcard) || !(keycards & it_redskull) ||
                                 !(keycards & it_bluecard) || !(keycards & it_blueskull) ||
                                 !(keycards & it_yellowcard) || !(keycards & it_yellowskull)))
            {
    ...
    if (ok == false)
        S_StartScreamSound(this, sfx_usefail);
...
void PlayerPawn::ProcessSpecialSector(sector_t *sector)
{
    if (sector->damage)
    {
        ...
        else if (!powers[pw_ironfeet] || (damage >= 20 && P_Random() < 5))
...
void PlayerPawn::PlayerInSpecialSector()
{
    ...
    while (node)
    {
        ...
        specialsector =
            sec->special; // Fab: keep track of what sector type the player's currently in
             ...
             onground))
        {
            ...
            if ((temp == SS_damage_32 && !(mp->maptic & 31)) ||
                (temp == SS_damage_16 && !(mp->maptic & 15)) || instantdamage)
            {
...
{
    static const weapontype_t GetAmmoChange[] = {wp_chaingun,
                                                 wp_shotgun,
                                                 wp_plasma,
                                                 wp_missile,
                                                 wp_goldwand,
    ...
    {
        if (ammo[weaponinfo[readyweapon].ammo] < weaponinfo[readyweapon].ammopershoot)
            UseFavoriteWeapon();
        return true;
    }
    else if (game.mode == gm_heretic)
    ...
    else
    {
        ...
        else
        {
            ...
            if (coop_multiplayer)
            {
                ...
                // TODO if you die, you lose all pieces gathered so far... leave them near the
                // corpse?
        ...
        // TODO Automatically check if we have all pieces... For now you have to assemble the
        // weapons yourself.
        /*
        if (all_pieces)
      {
