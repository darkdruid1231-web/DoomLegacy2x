...
//-----------------------------------------------------------------------------
//
// $Id: p_xpspr.cpp 533 2009-06-11 08:31:10Z smite-meister $
//
...
static int
MeleeBlow(PlayerPawn *player, int damage, float range, fixed_t power, angle_t sweep = ANG45 / 16)
{
    angle_t angle;
    float sine;
    Actor *targ;
...
void A_LightningZap(DActor *actor)
{
    ...
    DActor *mo = actor->mp->SpawnDActor(
        actor->pos.x + x, actor->pos.y + y, actor->pos.z + deltaZ, MT_LIGHTNING_ZAP);
    if (mo)
    {
        mo->owner = actor;
...
void A_ZapMimic(DActor *actor)
{
    ...
    if (ow)
    {
        if (ow->state >= ow->info->deathstate || ow->state == &states[S_FREETARGMOBJ])
        {
            actor->ExplodeMissile();
        }
        else
...
static bool PIT_BloodscourgeLook(Actor *mo)
{
    ...
    )
    {
        if (blocksearch_self->mp->CheckSight(blocksearch_self, mo))
        {
            ...
            angle_t angle =
                R_PointToAngle2(master->pos.x, master->pos.y, mo->pos.x, mo->pos.y) - master->yaw;
            angle >>= 24;
...
void A_CHolyAttack(PlayerPawn *p, pspdef_t *psp)
{
    ...
    float dummy;
    mo->target =
        p->AimLineAttack(p->yaw, AIMRANGE, dummy); // HACK to compensate SPMAngle without autoaim
...
static void CHolySeekerMissile(DActor *actor, angle_t thresh, angle_t turnMax)
{
    ...
    if (!(target->flags & MF_SHOOTABLE) || !(target->flags & MF_VALIDTARGET))
...
static void CHolyTailFollow(Actor *actor, fixed_t dist)
{
    ...
    if (child)
    {
        ...
        if (child
                ->TryMove(
                    actor->pos.x + dist * finecosine[an], actor->pos.y + dist * finesine[an], true)
                .first)
        {
            ...
            {
                child->pos.z =
                    actor->pos.z + (newDistance / oldDistance) * (child->pos.z - actor->pos.z);
            }
        }
    ...
    if (parent)
    {
        ...
        { // Ghost removed, so remove all tail parts
            ...
            return;
                     ...
                     ->TryMove(parent->pos.x - 14 * Cos(parent->yaw),
                               parent->pos.y - 14 * Sin(parent->yaw),
                               true)
                     .first)
        {
...
void A_FireConePL1(PlayerPawn *player, pspdef_t *psp)
{
    ...
    if (!conedone)
    {
        ...
        if (mo)
        {
            mo->special1 = SHARDSPAWN_LEFT | SHARDSPAWN_DOWN | SHARDSPAWN_UP | SHARDSPAWN_RIGHT;
