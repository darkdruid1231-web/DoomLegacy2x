...
//-----------------------------------------------------------------------------
//
// $Id: r_bsp.cpp 518 2007-12-31 02:54:58Z smite-meister $
//
...
int R_DoorClosed()
{
    return
        ...
        && (!backsector->SkyCeiling() || !frontsector->SkyCeiling());
}
...
sector_t *Rend::R_FakeFlat(
    sector_t *sec, sector_t *tempsec, int *floorlightlevel, int *ceilinglightlevel, bool back)
{
    ...
    if (ceilinglightlevel)
        *ceilinglightlevel =
            sec->ceilinglightsec == -1 ? sec->lightlevel : sectors[sec->ceilinglightsec].lightlevel;
    ...
    if (sec->heightsec != -1 && sec->heightsec_type == sector_t::CS_boom)
    {
        ...
        *tempsec = *sec;
        ...
        { // head-below-floor hack
            ...
            if (floorlightlevel)
                *floorlightlevel =
                    s->floorlightsec == -1 ? s->lightlevel : sectors[s->floorlightsec].lightlevel;
...
void Rend::R_Subsector(int num)
{
        ...
        (frontsector->heightsec != -1 && sectors[frontsector->heightsec].SkyCeiling()))
    {
        floorplane = R_FindPlane(frontsector->floorheight,
                                 ...
                                 floorcolormap,
                                 NULL,
                                 frontsector->SkyFloor());
    }
    else
        floorplane = NULL;

    if ((frontsector->ceilingheight > viewz || frontsector->SkyCeiling() ||
         (frontsector->heightsec != -1 && sectors[frontsector->heightsec].SkyFloor())))
    {
    ...
    //
    if (waterheight > frontsector->floorheight && waterheight < frontsector->ceilingheight)
    {
    ...
    for (ffloor_t *rover = frontsector->ffloors; rover && numffloors < MAXFFLOORS;
         rover = rover->next)
    {
             ...
             (viewz > *rover->bottomheight && (rover->flags & FF_BOTHPLANES))))
        {
            light = R_GetPlaneLight(
                frontsector, *rover->bottomheight, viewz < *rover->bottomheight ? true : false);
        ...
        {
            light = R_GetPlaneLight(
                frontsector, *rover->topheight, viewz < *rover->topheight ? true : false);
