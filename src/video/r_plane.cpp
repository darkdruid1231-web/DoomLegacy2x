...
visplane_t *Rend::R_FindPlane(fixed_t height,
                              Material *pic,
                              int lightlevel,
                              fixed_t xoff,
                              fixed_t yoff,
                              fadetable_t *planecolormap,
                              ffloor_t *ffloor,
                              bool sky)
{
    visplane_t *check;
    ...
    for (check = visplanes[hash]; check; check = check->next)
        if (height == check->height && pic == check->pic && lightlevel == check->lightlevel &&
            xoff == check->xoffs && yoff == check->yoffs &&
            planecolormap == check->extra_colormap && !ffloor && !check->ffloor &&
...
void R_DrawWaterSpan(void)
{
    ...
    {
        I_Error("R_DrawWaterSpan: %i to %i at %i", ds_x1, ds_x2, ds_y);
    }
    //      dscount++;
    // #endif
...
void Rend::R_DrawPlanes()
{
        ...
        for (pl = visplanes[i]; pl; pl = pl->next)
        {
	      ...
	      dc_colormap = fixedcolormap + base_colormap;
                ...
                for (x = pl->minx; x <= pl->maxx; x++)
                {
                    ...
                    {
                        fixed_t skycol;
                        skycol.setvalue((viewangle + xtoviewangle[x]) >>
                                        (ANGLETOSKYSHIFT - fixed_t::FBITS));
                        dc_x = x;
    ...
    VID_BlitLinearScreen(screens[0], screens[2], vid.width, vid.height, vid.width, vid.width);
