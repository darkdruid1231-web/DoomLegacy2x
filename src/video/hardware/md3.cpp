...
TGATexture::TGATexture(const char *n, int l) : LumpTexture(n, l, 0, 0)
{
...
bool MD3_t::Load(const string &filename)
{
    ...
    for (i = 0; i < nm; i++)
    {
        ...
        meshes[i].shaders =
            reinterpret_cast<MD3_mesh_shader *>(meshbase + meshes[i].header.ofsShaders);
        // for (int j=0; j<meshes[i].header.numShaders; j++) printf("mesh %d, shader '%s'.\n", j,
        // meshes[i].shaders[j].name);

        // triangles
        meshes[i].triangles =
            reinterpret_cast<MD3_mesh_triangle *>(meshbase + meshes[i].header.ofsTriangles);

        // texture coordinates (count == numVertices)
        meshes[i].texcoords =
            reinterpret_cast<MD3_mesh_texcoord *>(meshbase + meshes[i].header.ofsST);

        // and vertices (count == numVertices * numFrames)
        meshes[i].vertices =
            reinterpret_cast<MD3_mesh_vertex *>(meshbase + meshes[i].header.ofsVertices);
...
bool MD3_t::LoadSkinFile(const string &path, const string &filename)
{
    ...
    base[fc.LumpLength(lump) - 1] =
        0; // HACK, add NUL char at the end, overwrites part of last tag_ declaration...
    char *r = base;
...
MD3_player::MD3_player(const char *n) : cacheitem_t(n)
{
}
...
bool MD3_player::LoadAnim(const string &filename)
{
    ...
    int skip =
        anim[MD3_animstate::LEGS_WALKCR].firstframe - anim[MD3_animstate::TORSO_GESTURE].firstframe;
...
void MD3_animstate::Advance(MD3_anim *anim, float dt)
{
    const int next_looping[MAX_ANIMATIONS] = {
        BOTH_DEAD1,  BOTH_DEAD1,  BOTH_DEAD2,  BOTH_DEAD2,  BOTH_DEAD3,  BOTH_DEAD3,

        TORSO_STAND, TORSO_STAND, TORSO_STAND, TORSO_STAND, TORSO_STAND, TORSO_STAND, TORSO_STAND2,

        LEGS_WALKCR, LEGS_WALK,   LEGS_RUN,    LEGS_BACK,   LEGS_SWIM,   LEGS_IDLE,   LEGS_IDLE,
        LEGS_IDLE,   LEGS_IDLE,   LEGS_IDLE,   LEGS_IDLECR, LEGS_TURN};
...
#define Q2D_scale 0.75
    ...
    glTranslatef(pos.x.Float(),
                 pos.y.Float(),
                 pos.z.Float() + Q2D_scale * (mdl->legs.frames[st->frame].origin[2] + 24));
