...
//-----------------------------------------------------------------------------
//
// $Id: hwr_states.cpp 388 2006-11-04 14:48:17Z smite-meister $
//
...
TextureModifier::TextureModifier()
    : gen_s(GL_FALSE), gen_t(GL_FALSE), env_mode(GL_MODULATE), wrap_s(GL_REPEAT), wrap_t(GL_REPEAT),
...
Fog::Fog() : mode(GL_FALSE), density(1.0f), start(0.0f), end(1.0f)
{
...
{
    // Be carefull: Cg needs a valid OpenGL context (be sure no shader is created before we have an
    // OpenGL window)
    if (!cg_context)
    {
    ...
    if (shader_name.length())
    {
        ...
        fragment_program = cgCreateProgramFromFile(
            cg_context, CG_SOURCE, shader_name.c_str(), fragment_profile, "FragmentProgram", 0);
        CGerror fragment_error = cgGetError();
        if (fragment_error != CG_NO_ERROR)
        {
            printf("Warning: No valid fragment program found in %s: %s\n",
                   shader_name.c_str(),
                   cgGetErrorString(fragment_error));
            fragment_program = 0;
        }
        else
        {
...
void Shader::Apply()
{
    if (vertex_program)
    {
        cgGLSetStateMatrixParameter(cgGetNamedParameter(vertex_program, "ModelViewProj"),
...
State::State()
    : blend_func_src(GL_ONE), blend_func_dst(GL_ZERO), alpha_func(GL_ALWAYS), alpha_ref(0.0f),
      shade_model(GL_SMOOTH), cull_face_mode(GL_FALSE), fog(0)
#ifdef CG_SHADER
      ,
      shader(0)
#endif
...
void State::Apply()
{
    ...
    if (global_modulation)
    {
        ...
        if (modulation == MEDIUM)
        {
            ...
            modulated_color[3] =
                (global_modulation & ALPHA) ? (color[3] + global_color[3]) / 2.0f : color[3];
        }
        else if (modulation == MULTIPLY)
        {
            ...
            modulated_color[3] =
                (global_modulation & ALPHA) ? (color[3] * global_color[3]) : color[3];
        }
        ...
        else if (modulation == INVERSE)
        {
            ...
            modulated_color[3] =
                (global_modulation & ALPHA) ? (global_color[3] - color[3]) : color[3];
        }
