...
//-----------------------------------------------------------------------------
//
// $Id: r_anim.cpp 504 2007-12-16 17:58:26Z smite-meister $
//
...
class AnimatedMaterial
{
  ...
  public:
    ...
    AnimatedMaterial(Material *m,
                     const AnimatedMaterial *a,
                     int frame); // for constructing the slave copies
    ~AnimatedMaterial();
    ...
    void Update(tic_t t)
    {
        if (nextchange + 250 < t)
        {
            // skip too long intervals (also sort of handles the wrapping of tic_t, in case your
            // game runs more than 3.8 years continuously...:)
            nextchange = t + 10;
            return;
        }
...
AnimatedMaterial::AnimatedMaterial(Material *m, int n) : mat(m)
{
...
AnimatedMaterial::AnimatedMaterial(Material *m, const AnimatedMaterial *a, int f) : mat(m)
{
    ...
    while (p.NewLine())
    {
        ...
        if (state != PS_NONE)
        {
            ...
            if (temp != 'p' && temp != 'P')
            {
                ...
                // create the master copy, picking the frame Textures from the already generated
                // Materials
                AnimatedMaterial *m = new AnimatedMaterial(first, n);
                for (i = 0; i < n; i++)
                    m->frames[i] = frames[i];
