...
bool Wad::Open(const char *fname)
{
    ...
    waddir_file_t *temp =
        (waddir_file_t *)Z_Malloc(numitems * sizeof(waddir_file_t), PU_STATIC, NULL);
...
int Wad3::FindNumForName(const char *name, int startlump)
{
    ...
    {
        if (p->iname[0] == x[0] && p->iname[1] == x[1] && p->iname[2] == x[2] &&
            p->iname[3] == x[3])
            return j;
    }
    // not found
...
struct zip_central_directory_end_t
{
    ...
    Uint16 disk_number; // ZIP archives can consist of several files, but we do not support this
                        // feature.
...
bool ZipFile::Open(const char *fname)
{
    ...
    if (cd_end.disk_number != 0 || cd_end.num_of_disk_with_cd_start != 0 ||
    ...
    for (int k = 0; k < numitems; k++)
    {
        ...
        if (fh->signature[0] != 'P' || fh->signature[1] != 'K' || fh->signature[2] != '\1' ||
            fh->signature[3] != '\2')
        {
            // corrupted directory
        ...
        p += sizeof(zip_file_header_t) + n_size + SHORT(fh->extrafield_size) +
             SHORT(fh->comment_size);
        ...
        {
            CONS_Printf("Uncompressed lump '%s' in ZIP file '%s' has unequal compressed and "
                        "uncompressed sizes.\n",
                        directory[item].name,
                        fname);
            continue;
        ...
        if (lh.signature[0] != 'P' || lh.signature[1] != 'K' || lh.signature[2] != '\3' ||
            lh.signature[3] != '\4')
        {
        ...
        if (lh.flags != fh->flags || lh.compression_method != fh->compression_method ||
            lh.compressed_size != fh->compressed_size || lh.size != fh->size)
        {
            CONS_Printf("Local header for lump '%s' in ZIP file '%s' does not match the central "
                        "directory.\n",
                        directory[item].name,
                        fname);
            continue;
        ...
        directory[item].offset +=
            sizeof(zip_local_header_t) + SHORT(lh.filename_size) + SHORT(lh.extrafield_size);

        // accepted
        imap.insert(pair<const char *, int>(directory[item].name, item)); // fill the name map
        item++;
    }
    // NOTE: If lumps are ignored, there will be a few empty records at the end of directory. Let
    // them be.
    numitems = item;
    Z_Free(tempdir);
...
int ZipFile::Internal_ReadItem(int item, void *dest, unsigned size, unsigned offset)
{
    ...
    if (!l->deflated)
    {
        fseek(stream,
              l->offset + offset,
              SEEK_SET);                     // skip to correct offset within the uncompressed lump
        return fread(dest, 1, size, stream); // uncompressed lump
    }

    // DEFLATEd lump, uncompress it

    // NOTE: inflating compressed lumps can be expensive, so we transparently cache them at first
    // use. This way we only have to inflate the lump once. Uncompressed lumps are treated as usual.
    ...
    if (!cache[item])
        Z_Malloc(unpack_size,
                 PU_STATIC,
                 &cache[item]); // even if cache[item] is allocated, it is not yet filled with data
